...
POST /v1/client-config-packages/{package_id}/session
...
- Upload authorization accepts an existing V3 session, the operator token from `V3_CLIENT_ARTIFACT_TOKEN`, or a short-lived package-scoped `v3cs_*` token.
- A `v3cs_*` token can only upload a manifest whose tenant, user, client, datasets, and asset libraries stay inside its resolved configuration package. It cannot list, read, download, attach, or publish artifacts.
- The session exchange endpoint is server-to-server only. It requires `V3_CLIENT_CONFIG_BRIDGE_TOKEN`; token signing uses the separate `V3_CLIENT_SESSION_SIGNING_KEY`. Both secrets must contain at least 32 characters and remain outside Git and client responses.
...
5. The client-resolved package contains V3-owned references only. It must not contain `codex_control`, provider keys, broad V3 admin tokens, activation tokens, or bridge credentials.
6. Legacy operator-created records may still contain `codex_control` internally for migration compatibility, but the session exchange response strips it before delivery to a native client.

### Session Exchange

After Codex account and terminal activation, 1服务器 exchanges the V3 package reference bound to that account:

```http
POST /v1/client-config-packages/{package_id}/session
Authorization: Bearer <V3_CLIENT_CONFIG_BRIDGE_TOKEN>
...
The response contains `config_package`, a one-hour-or-shorter `v3cs_*` `session_token`, and `session_expires_at`. The bridge must verify that tenant and user match the V3-owned package. The native client stores the token in the operating-system credential store and caches only the secret-free package on disk.
...
The current client separates Codex control activation from V3 configuration:

```text
email account login on souleye.cc
...
-> codex-web exchanges the bound V3 package reference
-> client caches the secret-free package and v3cs_* upload session separately
```
...
4. Normal native artifact upload uses the short-lived package-scoped `v3cs_*` session. `V3_CLIENT_ARTIFACT_TOKEN` remains an operator/smoke credential and is never delivered to customers.
