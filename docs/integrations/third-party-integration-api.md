...
The generated output includes a package directory, a `.tar.gz` archive, a matching `.sha256` file, a sibling machine-readable `.release.json` validation report, a sibling human-readable `.release.md` summary, and a sibling `.delivery-manifest.json` delivery manifest. The package includes:
