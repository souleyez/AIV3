...
The sibling `<package>.release.json` file records the combined validation result for the package directory, archive, sidecar, generated time, V3 commit, and handoff manifest. The sibling `<package>.release.md` file summarizes the same release status, V3 commit, generated time, archive SHA256, checks, delivery-manifest provenance, and errors for human review. The sibling `<package>.delivery-manifest.json` file lists the expected delivery artifacts, including the expanded package directory, package manifest, archive, `.sha256` sidecar, release JSON report, and release Markdown summary with SHA256 values for receive-side verification; delivery validation also checks its package type, generated time, and V3 commit against the package manifest and release JSON. The sibling `<package>.all.json` and `<package>.all.md` files are generated after the delivery manifest and capture the full handoff, package, archive, delivery, and release gate result for review records, including package type, generated time, V3 commit provenance, and package/archive HTML command-contract readiness. Final evidence validation checks that `<package>.all.md` is freshly rendered from the current aggregate validation output, not only that its file hash appears in the evidence manifest. The package builder checks the aggregate receipt freshness and returns `allMarkdownReady`. The sibling `<package>.evidence-manifest.json` file is generated last and records every final handoff artifact plus the aggregate evidence sidecars without creating a circular delivery-manifest hash dependency; it also carries `all_markdown_ready=true` and `html_artifact_command_contract_ready=true` so the final evidence manifest itself exposes aggregate Markdown receipt and HTML command-contract readiness. Final evidence validation requires the package type, generated time, V3 commit provenance, `all_ready`, `all_markdown_ready`, and `html_artifact_command_contract_ready` to be valid and consistent with the package manifest, release JSON, delivery manifest, and aggregate JSON before the package is considered ready. The sibling `<package>.evidence-manifest.md` file is a human-readable receipt rendered from final evidence validation, including package type, generated time, V3 commit, manifest SHA256, artifact count, aggregate Markdown receipt freshness, package/archive HTML artifact readiness, HTML command-contract readiness, and error codes. The builder checks this receipt for freshness and returns `evidenceMarkdownReady`. The V3 safe HTML artifact manifest also names the package scripts behind `npm run validate:all` and `npm run validate:evidence`, including `--verifyMarkdown auto` and the receipt-freshness checks each command is expected to cover.
...
`validate:package` checks the package manifest, package type, generated time, V3 commit, relative-root safety, file SHA256 values, handoff manifest readiness, and the V3 safe HTML artifact manifest contract. That manifest contract requires `validate:all` and `validate:evidence` to declare their receipt-freshness coverage and package scripts with `--verifyMarkdown auto`, so the review page cannot drift from the actual recipient-side gate behavior.
...
This checks the `.sha256` sidecar, gzip/tar structure, single package root, path traversal safety, required entries, package manifest file hashes, archived V3 safe HTML artifact manifest contract, handoff manifest readiness, and the final aggregate receipt signals that prove the package and archive both kept the review-friendly HTML artifact ready.
