...
- 包目录同级的 `<package>.all.json` 和 `<package>.all.md`，在交付清单生成后记录交接清单、包完整性、归档、交付清单和 release gate 的完整聚合校验证据，并带出 package type、生成时间和 V3 提交 provenance；最终 evidence 校验会确认 `<package>.all.md` 确实由当前聚合校验结果重新渲染得到，而不仅是文件 hash 出现在证据清单里。
