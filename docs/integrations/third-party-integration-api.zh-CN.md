...
- `.mp4`、`.mov`、`.m4v`、`.webm`、`.mkv`、`.avi` 可作为视频素材上传或登记；推荐传准确的 `video/*` MIME 类型，并在 `title` 或 `content_url` 中保留真实文件后缀。
- 视频素材会先按普通资料完成登记、下载和基础解析状态流转。是否能作为普通问答证据，取决于后续是否已产生可检索的转写、OCR 或切片证据；不要把视频登记成功等同于已经完成 PPT 抽取。
- 从视频中抽取 PPT/幻灯片/课件页是特殊触发能力。第三方可在后续聊天事件中授权该文档或数据集范围，并让用户明确提出“提取这个视频里的 PPT/幻灯片/课件”；DataMax 会进入 `VideoExtraction` 后台流程并返回状态卡或产物链接。原文、字幕或转写可以作为视频 PPT 抽取任务的附带交付物，但单纯“提取字幕/转写原文”不应触发 PPT 抽取。
- 该能力只面向视频里已经播放 PPT/课件的内容，不负责把普通视频创作成 PPT。微信视频号、扫码登录、cookie、私有平台页或需要登录态的来源不属于自动解析范围；系统不会声称已经看过这类视频或已经生成 PPT。请改为上传视频文件、提供可匿名下载的直连视频 URL，或提供公开页面链接且页面 HTML 暴露 `<video>`/`<source>`、OpenGraph、Twitter video、JSON-LD `contentUrl`/`embedUrl` 中的匿名直连视频；确有授权但拿不到视频文件时，可申请 operator 授权录屏处理。
