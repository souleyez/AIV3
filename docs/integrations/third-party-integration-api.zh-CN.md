...
- 包目录同级的 `<package>.evidence-manifest.json`，最后生成，用于列出最终交付物和聚合证据文件，避免把 `.all.json` 反写进 delivery manifest 造成循环 hash；最终 evidence 校验也会要求 package type、生成时间和 V3 提交 provenance 合法，并与包内清单、release JSON、delivery manifest、aggregate JSON 一致，全部通过后才认为交付包 ready。
