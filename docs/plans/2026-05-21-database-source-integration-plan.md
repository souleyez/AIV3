...
- Database sync can resolve an effective target dataset from request `dataset_id`, request `dataset_external_id` / `dataset_title`, or source `database_source.default_dataset_id`.
...
- MySQL incremental sync now applies table-level checkpoints when fetching rows, supports high-water filtering by mapped `updated_at_column`, `version_column`, or numeric single-column `id_column`, writes the next table high-water marks back to `external_sync_runs.checkpoint`, and reuses the latest successful checkpoint when an incremental sync request omits an explicit checkpoint.
- MySQL document fetch now has an internal `page_size` control, fetches mapped rows in bounded pages up to `row_limit`, keeps converting later rows after a single row conversion failure, and carries real row/document/skipped/failed counts plus table-level counts into the external-source worker output and sync-run counts. It also persists a capped, secret-free set of row-failure samples with table, row index, optional source primary key, and redacted reason so the selected-source status route and centralized observability page can show why rows were skipped without keeping the database connection open.
- The selected-source status route now emits a secret-free `health_findings` summary. It groups config invalidity, missing/default dataset issues, explicit target dataset detection, missing semantic profile, semantic-profile/table-mapping gaps, low mapping confidence, mapped tables without documents, indexing failures/in-progress states, sync failures, no-document syncs, and row conversion failures into blocking/warning/info findings. The centralized observability page displays this only for the selected source.
...
- Database source auto-bind/auto-create dataset behavior now matches document parse `dataset_external_id` behavior for the main-system sync route. The remaining P1 question is whether to expose the same shape through a third-party-safe database source API.
- Sync status now has a third-party-safe read-only query surface for sources explicitly allowed by a channel. Remaining status work: field-level compatibility review with customer clients, richer grouping/export for row failures, and optional per-sync-id detail once sync launch becomes public.
...
- Integration observability still needs deeper database drilldowns after the selected-source summary: live table/schema drift against latest inspected schema, richer incremental conflict/detail views, row-failure grouping/export, semantic-profile refresh actions, and third-party-safe status views.
- Public third-party docs now include the managed P0 database status query. Full third-party database registration/sync docs should wait until those APIs are explicitly opened.
