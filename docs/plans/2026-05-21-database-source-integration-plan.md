...
- Internal operators can explicitly refresh and apply a database semantic profile from both the workspace data-source panel and the centralized integration observability page. The refresh uses the existing main-system `database/apply-profile` route only after a button click, then reloads the selected status summary; it does not add a third-party public API or run while the page is idle.
...
- MySQL document fetch now has an internal `page_size` control, fetches mapped rows in bounded pages up to `row_limit`, keeps converting later rows after a single row conversion failure, and carries real row/document/skipped/failed counts plus table-level counts into the external-source worker output and sync-run counts. It also persists a capped, secret-free set of row-failure samples with table, row index, optional source primary key, and redacted reason; the selected-source status route derives table/reason `row_failure_groups` from those counts so operators can see failure scale before opening samples, without keeping the database connection open.
