...
**目标：** 让 8 服务器上的 DataMax 在演示环境中稳定调用 1 服务器 `codex-web` / Cloudflare Codex，完成“第三方或本地指令 -> Image2 效果图 -> Codex 生成最终 HTML -> DataMax 发布产物链接”的闭环。

**原则：** DataMax 仍然是系统事实源。8 服务器负责对话、数据集、文档范围、任务编排、状态、发布和第三方响应；1 服务器 / Cloudflare Codex 只作为受控执行器，不直接成为第三方入口、权限中心或业务状态中心。
...
3. 最终 HTML 生成不应只停留在 8 侧本地 renderer。
   ...
   - 8 服务器负责把 Cloudflare 返回的结构化结果或 `artifact.html` 发布到 DataMax 的 `/generated-artifacts/`。
...
5. 任务结果可以返回：
   - Image2 图片 artifact；
   - 最终 HTML；
   - 或 DataMax 可校验的 generated-artifact public URL。
...
- DataMax 在效果图 ready 后提交 `static_page_image2_data_publish`。
- Cloudflare Codex 返回以下任一结果：
  - `artifact.public_url`
  - `artifact.html`
- 若返回 `artifact.html`，8 服务器负责写入 DataMax generated-artifacts 目录并生成 public URL。
- 校验最终 URL：
  - HTTP 200；
  - HTML 完整；
...
- 只有 Cloudflare 明确 `failed/cancelled` 或 DataMax 校验不通过才标 failed。

建议：

- 演示期将 `STATIC_PAGE_ORCHESTRATOR_MAX_POLLS` 调到至少 240，按 5 秒轮询约 20 分钟。
- 演示期将 `CODEX_HOST_AGENT_TASK_TIMEOUT_MS` 调到至少 1800000，避免 Cloudflare Codex 生成静态页超过 15 分钟时被 DataMax 侧提前判失败。
...
- 慢任务不会误报失败。
- 观测页能定位失败在哪一段：DataMax 编排、Image2、Cloudflare Codex、发布。
...
- 不把 Cloudflare Codex 作为 DataMax 权限中心。
...
2. `codex-host-agent` 的 Cloudflare output 继续保持“HTML 由 DataMax 发布”的路径。
