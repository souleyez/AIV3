...
- Current implementation follow-up tightens package-build output semantics: `handoffReady` now names the sample handoff manifest gate, while top-level `ready` means the final sendable package is ready across handoff, release, aggregate, and evidence gates.
