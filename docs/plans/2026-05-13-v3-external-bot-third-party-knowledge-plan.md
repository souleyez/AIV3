...
- Current package/archive-validation follow-up gates that HTML artifact manifest inside `validate:package`, `validate:archive`, `validate:release`, and `validate:all`, checking the V3 template/source contract, read-only mode, default domain, required event endpoint, validation command listing, and unsafe payload strings even when the recipient validates only the `.tar.gz`.
