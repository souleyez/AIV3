...
- V3 awareness/search checkpoint now adds a shared additive model-awareness policy to the startup briefing and backend AssistantRun provider input/continue input, and external channel AssistantRun startup briefing persists the same model-awareness and visible-scope policy for observability/Codex context reuse. External chat surfaces that use the normal AssistantRun path inherit V3 identity, visible-scope discipline, `当前不可见/未供料` wording, no-fake-live-search behavior, and a read-only `web_search` action contract that requires V3 validation plus source/time search evidence before citation.
...
- Current implementation follow-up tightens package-manifest provenance at the root of the handoff chain: `validate:package` now requires `generated_at` to be an ISO UTC timestamp and `repository_head` to be a short or full git SHA before downstream delivery/release/evidence gates can rely on those fields.
...
- Current implementation follow-up tightens that delivery gate with package provenance checks: delivery manifests now expose and validate `package_type`, `generated_at`, and `repository_head` against both the package manifest and release JSON, and the release Markdown receipt surfaces delivery provenance for human review.
...
- Current implementation follow-up tightens that final evidence manifest gate by requiring `package_type=v3.external_third_party_handoff_package.v1`, so the final evidence receipt cannot be reused for another artifact family.
- Current implementation follow-up surfaces that package type in the human-readable `<package>.evidence-manifest.md`, so reviewers see the same artifact-family boundary that the machine gate enforces.
- Current implementation follow-up also surfaces final evidence `generated_at` and `repository_head` provenance in the human-readable evidence receipt, matching the release receipt's operator review shape.
- Current implementation follow-up validates final evidence provenance directly: `validate:evidence` now requires `generated_at` to be an ISO UTC timestamp and `repository_head` to be a short or full git SHA before the handoff package can be considered ready.
- Current implementation follow-up also cross-checks final evidence provenance against the package manifest, release JSON, and delivery manifest, so a valid-looking final evidence manifest cannot drift from the package source of truth.
- Current implementation follow-up adds a human-readable final evidence receipt: package builds now also write sibling `<package>.evidence-manifest.md`, and `validate:evidence --markdown <file>.md` can regenerate the same review summary from final evidence validation.
- Current implementation follow-up tightens package-build output semantics: `handoffReady` now names the sample handoff manifest gate, `evidenceMarkdownReady` verifies the human-readable evidence receipt matches the current validation output, and top-level `ready` means the final sendable package is ready across handoff, release, aggregate, evidence, and evidence-receipt gates.
- Current implementation follow-up gates readiness on final evidence manifests and their human-readable receipts: readiness reports generated with `--releasePackage` now run `validate:evidence`, verify the sibling `<package>.evidence-manifest.md`, add `handoff_evidence_ready`, and include a redacted `handoff_evidence_summary` plus Markdown summary section.
- Current implementation follow-up makes generated package-internal `npm run validate:evidence` use `--verifyMarkdown auto`, so third-party recipients automatically verify the sibling human-readable evidence receipt without manually passing its path.
- Current implementation follow-up adds a CLI-level regression test for `validate:evidence --verifyMarkdown auto`, proving the package-recipient command can infer and verify the sibling human-readable evidence receipt from the final evidence manifest path.
...
- Current implementation follow-up surfaces those pending search-evidence states in observe-first audit: `GET /v1/external/integrations/{id}/audit?item_type=search_evidence` returns redacted `web_search` evidence-required rows, and the standalone panel adds a `搜索证据` filter without exposing raw user queries.
...
- Current package/archive-validation follow-up gates that HTML artifact manifest inside `validate:package`, `validate:archive`, `validate:release`, and `validate:all`, checking the V3 template/source contract, read-only mode, default domain, required event endpoint, `validate:all` plus `validate:evidence` command listing, and unsafe payload strings even when the recipient validates only the `.tar.gz`.
- Current final-evidence follow-up now requires `.all.json` to carry package/archive `html_artifact_ready` signals and exposes those signals in `handoff_all_summary` / `handoff_evidence_summary`, so the customer handoff evidence fails closed if the review-friendly HTML artifact disappears from either the package directory or sendable archive.
- Current aggregate-evidence follow-up surfaces `package_type`, `generated_at`, and `repository_head` in `validate:all` JSON/Markdown receipts, and `validate:evidence` cross-checks the aggregate JSON provenance against the package manifest, release JSON, and delivery manifest before final handoff evidence can pass.
...
- Current command-contract evidence follow-up propagates that HTML artifact command-contract readiness into `validate:all`, `validate:release`, `<package>.evidence-manifest.json`, and `validate:evidence` Markdown/JSON summaries, so final handoff evidence fails closed if aggregate reports omit the package/archive command-contract readiness signals.
