...
| 顺序 | 当前项 | 状态 | 下一步 | 关闭标准 |
| --- | --- | --- | --- | --- |
| 1 | P5 chat message manifest hydrate helper 拆分 | 本地已完成行为保持拆分和 P0 验证。 | 提交 GitHub，部署 8 服务器，跑 8 服务器 P0 smoke，再把远端验证写回 validation。 | `chat_session_manifest_view_support` 模块单测、pending chat session/chat message 旧回归、`cargo check`、Web build、P0 smoke、8 服务器 release build、health/ready 和服务 active 全通过；不改变第三方公开契约。 |
