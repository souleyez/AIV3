...
| 顺序 | 当前项 | 状态 | 下一步 | 关闭标准 |
| --- | --- | --- | --- | --- |
| 1 | P5 external answer policy helper 拆分 | 已完成行为保持拆分、提交 GitHub、部署 8 服务器并通过 P0 验证。 | 已写回远端验证；作为当前已关闭基线。 | `external_answer_policy_support` 模块单测、原有 answer policy 回归、第三方 payload 别名、`external_channel_static_page_artifact`、`cargo check`、Web build、P0 smoke、8 服务器 release build、health/ready 和服务 active 全通过；不改变第三方公开契约。 |
| 2 | P5 external artifact/requested skills helper 拆分 | 待处理。 | 如果没有 P1 凭证、P2/P4 写入确认或新客户失败样例，下一步评估第三方 artifact/requested skills 解析 helper 的行为保持拆分。 | 有清晰模块边界、聚焦测试和 P0 smoke；不改变第三方公开契约。 |
