...
5. quality matrix 显式复核风险收紧已完成：`manual_review_required`、`missing_transcript_alignment`、`frame_sharpness_review_required` 和 `slide_readability_review_required` 会保持 `needs_manual_review`，避免只有 quality score 达标就误判 public sample 可交付。
