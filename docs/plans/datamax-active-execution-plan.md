...
| 顺序 | 当前项 | 状态 | 下一步 | 关闭标准 |
| --- | --- | --- | --- | --- |
| 1 | P5 manifest service handoff parser helper 拆分 | 已完成本地验证、GitHub 同步、8 服务器 code deploy、服务重启、health/ready 和私有 smoke。 | 本项关闭；下一步默认继续 P5 小切片，或在拿到 P1/P2/P4 条件或新 P3 客户样例时切换优先级。 | `manifest_service_handoff_support` 模块单测、旧 dataset output/report/chat handoff 回归、`cargo check`、Web build、P0 smoke、8 服务器 release build、health/ready 和服务 active 全通过；不改变第三方公开契约。 |
