...
| 顺序 | 当前项 | 状态 | 下一步 | 关闭标准 |
| --- | --- | --- | --- | --- |
| 1 | P5 tool view helper 拆分 | 已完成本地验证；待提交、同步 GitHub、部署 8 服务器并跑 8 服务器 P0 smoke。 | 提交本切片，按 code deploy 边界同步 8 服务器、构建、重启并验证。 | `cargo test -p platform-api tool_view_support --lib`、`cargo test -p platform-api to_tool_definition_view --lib`、`cargo test -p platform-api to_tool_execution_view --lib`、`cargo fmt --check`、`cargo check -p platform-api`、Web build、P0 smoke 全通过；validation 写明未改公开接口/鉴权/字段；提交可单独回退。 |
...
1. 先关闭当前 P5 tool view helper 拆分：提交、推送、同步 8 服务器、release build、重启五个服务、跑 health/ready 和 P0 smoke。
