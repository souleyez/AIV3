...
- P2-2E 三样例质量矩阵 self-test scaffold、本地 `generated_artifacts/` 输入适配、public-course deliverables 分类入口和 customer-authorized deliverables 输入入口已完成；quality matrix 可用 `--synthetic-deliverables <path>` 复核合成类本地产物，用 `--public-course-deliverables <path>` 把匿名公开视频样例归到 `public_course_video`，也可在获得明确授权后用 `--customer-deliverables <path>` 复核客户/operator 授权样例。
...
| 主站上传视频 | `smoke:video-ppt-upload-main` 已实现并本地验证脚本契约 | live controlled smoke 待用户批准，因为会写一条非客户 smoke 记录 | 执行包 EP2 |
...
`--customer-deliverables` 已补齐，但只表示脚本能够复核一个已授权的客户/operator 样例交付包；没有真实授权输入时，customer gate 仍是 pending，不得用 public 或 synthetic 样例冒充客户样例。
...
| M6H | EP5 customer-authorized deliverables 质量矩阵入口 | 无 live 授权；只用本地非客户 fixture 验证脚本契约 | 已通过 `node --check scripts/smoke/video-ppt-quality-matrix.mjs`、help、自测、synthetic/public/customer 三种 deliverables 输入模式、self-test/customer 多输入负向门禁 | 已完成：`--customer-deliverables <generated_artifacts>` 可把已授权样例归入 `customer_authorized_video`，报告仍保持 `matrix_complete=false`，真实客户样例仍必须等待授权输入 | 未部署 |
