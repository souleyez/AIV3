...
| 主站上传视频 | `smoke:video-ppt-upload-main` 已实现，并已补离线 self-test 验证 artifact/download contract 和视频扩展识别 | live controlled smoke 待用户批准，因为会写一条非客户 smoke 记录 | 执行包 EP2 |
...
| 主站上传入口 | 脚本已实现，离线 self-test 已补；live smoke 未跑 | 阻塞“上传入口已验收”结论 | 需要用户批准写一条非客户 smoke 记录 |
...
- `npm run smoke:video-ppt-upload-main` 已纳入 package scripts 和 `scripts/README.md`；脚本语法、help、离线 self-test、upload classifier、HTML artifact manifest 和 video deliverable validator 均已通过本地验证。
...
| 主站上传视频 | 用户上传 `.mp4/.mov/.m4v/.webm/.mkv/.avi` | 上传登记后明确触发 PPT 抽取，并通过 artifact file API 下载 | `smoke:video-ppt-upload-main` 已实现并通过语法/help/self-test 检查；live/controlled 回执待授权 | P1-3B |
