...
| 235 | P5 client artifact public HTML publish response helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `client_artifact_publish_support` 新增 `publish_client_artifact_public_html_and_load_response` 和 response builder，把客户端产物公开 HTML 发布中的 `artifact_id` 必填校验/trim、公开 HTML 发布 view 加载和 `PublishClientArtifactPublicHtmlResponse` 构造从 `publish_client_artifact_public_html` 下沉；handler 仍负责用户 session、`Utc::now()` 时间戳生成和 `Json` 响应包装，公开 URL、HTML artifact manifest、owner 校验、status/manifest 更新、响应字段和错误码保持不变；`cargo fmt --check`、`cargo test -q -p platform-api client_artifact_publish_support --lib`、`cargo test -q -p platform-api client_artifact --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
| 236 | P5 client artifact HTML preview validation helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `client_artifact_publish_support` 新增 `validate_client_artifact_html_preview_request`，把客户端产物 HTML 预览中的 `artifact_id` 必填校验和 file index 校验从 `preview_client_artifact_html_file` 下沉；handler 仍负责用户 session、路径参数传入和 `Response` 返回，preview 文件加载仍使用原始路径 `artifact_id`、owner 校验仍使用 trim 后资源 ID，预览大小门禁、HTML 清洗、响应头、错误码和公开契约保持不变；`cargo fmt --check`、`cargo test -q -p platform-api client_artifact_publish_support --lib`、`cargo test -q -p platform-api client_artifact --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
| 237 | P5 client artifact create response helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `client_artifact_create_support` 新增 `create_client_artifact_from_upload_response` 和 response builder，把客户端产物创建中的 `CreateClientArtifactResponse` 构造从 `create_client_artifact` 下沉；handler 仍负责 upload/session 鉴权、multipart 解析和 `201 Created` 包装，manifest/files 校验、scope 校验、`v3ca_` ID 生成、文件存储、事务提交、回读 view、响应字段和错误码保持不变；`cargo fmt --check`、`cargo test -q -p platform-api client_artifact_create_support --lib`、`cargo test -q -p platform-api client_artifact --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
| 238 | P5 client artifact get view helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `client_artifact_view_support` 新增 `get_client_artifact_view_for_request`，把客户端产物详情查询中的 view 加载从 `get_client_artifact` 下沉；handler 仍负责 upload/session 鉴权、路径参数传入和 `Json` 响应包装，`load_client_artifact_view` 的 artifact id trim、not_found 错误、文件列表加载、响应字段和公开契约保持不变；`cargo fmt --check`、`cargo test -q -p platform-api client_artifact_view_support --lib`、`cargo test -q -p platform-api client_artifact --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
| 239 | P5 client config package get view helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `client_config_package_support` 新增 `get_client_config_package_view_for_request`，把客户端配置包详情查询中的 view 加载从 `get_client_config_package` 下沉；handler 仍负责路径参数传入和 `Json` 响应包装，`load_client_config_package_view` 的 package lookup、not_found 错误、payload 解码、响应字段和公开契约保持不变；`cargo fmt --check`、`cargo test -q -p platform-api client_config_package_support --lib`、`cargo test -q -p platform-api client_artifact --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
| 240 | P5 asset library membership path helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `asset_library_membership_support` 新增 `upsert_asset_library_dataset_membership_path_and_load_response`、`remove_asset_library_dataset_membership_path_and_load_response` 和 path id 解析 helper，把资产库数据集 membership upsert/delete 的 `asset_library_id`、`dataset_id` 路径解析从两个 handler 下沉；handler 仍负责用户 session、路径原文传入和 `Json` 响应包装，解析顺序、错误码、可见数据集/owner 校验、membership 写入/删除、响应字段和公开契约保持不变；`cargo fmt --check`、`cargo test -q -p platform-api asset_library_membership_support --lib`、`cargo test -q -p platform-api asset_library --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
| 241 | P5 asset library scope summary path helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `asset_library_scope_summary_support` 新增 `parse_asset_library_scope_summary_path`，把资产库 scope summary 的 `asset_library_id` 路径解析从 `get_asset_library_scope_summary` 下沉；handler 仍负责用户 session、header 读取、scope 数据加载/过滤和 `Json` 响应包装，invalid id 错误码、可见数据集过滤、membership scope 计算、资产供料加载、响应字段和公开契约保持不变；`cargo fmt --check`、`cargo test -q -p platform-api asset_library_scope_summary_support --lib`、`cargo test -q -p platform-api asset_library --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
...
1. 默认继续 #242：选择下一个 `platform-api` 或主站前端行为保持小切片，先补定向测试，再做拆分和 P0 回归。
