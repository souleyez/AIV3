...
| 顺序 | 当前项 | 状态 | 下一步 | 关闭标准 |
| --- | --- | --- | --- | --- |
| 1 | P5 assistant turn metadata hydrate helper 拆分 | 本地行为保持拆分和 P0 回归已通过；待提交、推送、部署 8 服务器。 | 提交 GitHub，部署 8 服务器，跑远端 release build、health/ready 和 P0 smoke，并写回 validation。 | `chat_session_manifest_view_support` 模块单测、pending chat session/chat message 旧回归、`cargo check`、Web build、P0 smoke、8 服务器 release build、health/ready 和服务 active 全通过；不改变第三方公开契约。 |
| 2 | 是否同步 8 服务器 | 待执行代码部署。 | 代码提交后按 code deploy 路径同步 8 服务器；验证通过后再做 docs-only 回写同步，不重启服务。 | 8 服务器 HEAD 与 GitHub `main` 一致；服务全 `active`；`/healthz`、`/readyz` 正常；8 服务器验证结果写入 validation。 |
