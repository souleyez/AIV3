...
- P2-2A/B/C/D/F 的本地质量切片已完成：质量报告、bright-canvas detector、讲师小窗/外部前景 crop、暗色/亮色/纯色低信息过滤、短动画转场过滤、OCR evidence 经 redaction 后进入 notes/Markdown/PPTX speaker notes、缺 OCR evidence 的低严重度风险提示、清晰度/可读性风险提示均有本地验证。
...
3. 更明确的 crop detector：针对 full-frame fallback 仍可见但边界不稳定的 public 页做 fixture。
4. 清晰度/readability 分流：把低清晰、低对比和 fade 页更明确标为人工复核。
5. OCR/subtitle 证据增强已补本地回归：只把 OCR 当作 page notes evidence，经过 `video_safe_evidence_text` 脱敏后写入 slide notes、`video_slides.md` 和 PPTX speaker notes；没有 transcript 时不生成或伪装 `subtitle_page_map`。
6. validator/shared JSON 输出脱敏已完成：内部 `validateVideoDeliverables()` 返回可供脚本读取的本地路径，但 CLI `--json` 和可复制共享报告默认把 `artifactsDir`、`files[].path` 等本机路径替换为 redacted marker，只保留 file kinds、summary counts、warning/error code 和 verdict。
7. 更多匿名公开视频课程样例：只选公开、非登录态、可下载、包含课件画面的短样例。
