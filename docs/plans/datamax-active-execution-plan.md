...
5. validator/shared JSON 输出脱敏已完成：内部 `validateVideoDeliverables()` 返回可供脚本读取的本地路径，但 CLI `--json` 和可复制共享报告默认把 `artifactsDir`、`files[].path` 等本机路径替换为 redacted marker，只保留 file kinds、summary counts、warning/error code 和 verdict。
