...
**当前性质：** 开发执行版；当前 GitHub 功能基线以 `git log -1` 为准，本文件记录到 M6BG live readiness command-template evidence。第 0.7.22 节是当前最新 plan-only 可执行方案入口，并保留 M6BF/M6BG 收版前的工作区隔离规则作为后续切片模板；第 0.7.21 节保留上一版完整收口；第 0.7.18-0.7.20 节保留完整背景、命令和历史口径，但若与第 0.7.22 节冲突，以第 0.7.22 节为准。当前公开视频样例仍应作为 `needs_manual_review` 真实样例，不强行标为 clean deliverable；本轮仍不部署 8 服务器。
**状态摘要：**

- 本轮已先完成 plan-only 收口，并继续按 P1/no-live 代码切片收掉 M6AV/M6AW/M6AX/M6AY/M6AZ/M6BA/M6BB/M6BC/M6BD/M6BE/M6BF/M6BG；第 0.7.22 节已把原计划、视频测试提取、提取效果复核、视频号/登录态 handoff、授权录屏兜底、GitHub 同步和 8 服务器部署门槛压成 P0-P8 可执行路径。没有跑 live、没有上传视频、没有发第三方事件、没有录屏、没有部署或触碰 8/120 服务器。后续默认仍按 P1/no-live 推进；任何 P2/P3/P4/P5/P6/P7 都需要对应授权、凭据、样例或部署窗口。
...
| 已完成开发基线 | 本文件记录到 M6BG；客户 quality matrix 参数门禁、保留策略门禁、readiness summary、supported extension evidence、main/external artifact evidence、pending gate requirements、live 写入授权负向门禁、live approval self-test gate、live approval command-template evidence 和 live readiness command-template evidence 均可审计 | 继续以 `git log -1` 和当次验证为准；不把历史记忆当 live 状态 |
