...
| 已完成开发基线 | 本文件记录到 M6BI；客户 quality matrix 参数门禁、保留策略门禁、readiness summary、supported extension evidence、main/external artifact evidence、pending gate requirements、live 写入授权负向门禁、live approval self-test gate、live approval command-template evidence、live readiness command-template evidence、command-template self-test contract 和 approval request summary 均可审计 | 继续以 `git log -1` 和当次验证为准；不把历史记忆当 live 状态 |
...
| 项 | 当前口径 | 执行要求 |
| --- | --- | --- |
| GitHub 功能基线 | 以 `git log -1` 为准；M6BI 收版后最新功能基线包含 approval request summary gate | 对外说“已完成”时必须引用已推送 commit 或当次验证，不能把工作区草稿说成已上线 |
