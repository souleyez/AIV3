...
| 顺序 | 当前项 | 状态 | 下一步 | 关闭标准 |
| --- | --- | --- | --- | --- |
| 1 | P5 external document metadata/key helper 拆分 | 本地已完成行为保持拆分和 P0 回归。 | 提交 GitHub 后按 code deploy 边界同步 8 服务器，跑远端定向回归和 P0 smoke。 | `external_document_object_support` 模块单测、外部文档数据集 lookup/update 回归、`external_channel_static_page_artifact`、`cargo check`、Web build、P0 smoke、8 服务器 release build、health/ready 和服务 active 全通过；不改变第三方公开契约。 |
