...
| 107 | P5 static-page module sample data helper 拆分 | 本地行为保持拆分和 P0 回归已通过；待提交、GitHub 同步和 8 服务器发布验证。 | 提交代码与验证文档，发布时在 8 服务器补 Web build、release build、服务 active、health/ready 和 P0 smoke。 | `static_page_module_sample_data_support` 模块单测、`static_page` 广义回归、`cargo check`、第三方报表/导出/临时文档/视频 PPT、静态页 5 路、Cloudflare fallback、prewarm observability self-test 和前端 Node 回归已通过；本机 Web build 仍因本地缺 `next` 未进入构建，需以 8 服务器 Web build 作为发布证据；不改变第三方公开契约。 |
| 108 | P5 下一个行为保持小切片 | 待选定。 | 从 `platform-api` 或主站前端中选择下一个低风险 helper/UI 子模块，先补定向测试，再拆分。 | 定向测试、`cargo fmt --check`、`cargo check`、Web build、P0 smoke、本地或 8 服务器按发布范围验证通过；提交可单独回退；不改变第三方公开契约。 |
