...
| 无授权本地准备度 | `No-Auth Live-Preflight Acceptance Rollup` 已通过；后续 validator 已扩展到 27 项，覆盖 PPTX notes XML 脱敏、selected manifest、PPTX/Markdown/selected_count 页数一致性、summary counts 和 shared JSON 脱敏 | 可宣称“无授权本地 gate 已过” | 作为 live 前基线保留；不替代 live/customer/deployment gates |
...
| 质量矩阵复核风险 gate | M6AA 已完成：quality matrix self-test 报告暴露 8 类 review-required risk flags，`missing_ocr_evidence`、`single_slide_output_review_required`、`full_frame_rectangle_fallback`、`selected_slide_duplicates_removed` 等都会保持 `needs_manual_review` | 可宣称“质量矩阵不会把显式复核风险误判为 clean deliverable” | 后续新增 media-worker risk flag 时，先扩展该 gate 并复跑 no-live rollup |
...
| --- | --- | --- | --- | --- |
| M7 无授权基线守护 | 没有 live/部署/客户授权 | 跑 no-live rollup、定向测试、public evidence 复核、文档台账 | 18/18 rollup 或对应定向 gate 通过；无 `target/` tracked | 需要 live 才能继续时立即停止并请求授权 |
