...
| 顺序 | 当前项 | 状态 | 下一步 | 关闭标准 |
| --- | --- | --- | --- | --- |
| 1 | P5 external bot message payload normalization helper 拆分 | 已完成行为保持拆分、提交 GitHub、部署 8 服务器并通过 P0 验证。 | 已写回本轮远端验证；下一步继续 P5 下一个小切片，或在拿到凭证/确认后切回 P1/P2/P4。 | `external_bot_message_payload_support` 模块单测、第三方 payload alias 旧回归、`external_channel_static_page_artifact`、`cargo check`、Web build、P0 smoke、8 服务器 release build、health/ready 和服务 active 全通过；不改变第三方公开契约。 |
