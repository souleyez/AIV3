...
1. 先关闭当前 P5 `runtime_manifest_support` 拆分：提交、同步 GitHub、8 服务器 code deploy、跑 8 服务器 P0 smoke 并补 validation。
