...
6. OCR/subtitle 证据增强已补本地回归：只把 OCR 当作 page notes evidence，经过 `video_safe_evidence_text` 脱敏后写入 slide notes、`video_slides.md` 和 PPTX speaker notes；没有 transcript 时不生成或伪装 `subtitle_page_map`。
7. validator/shared JSON 输出脱敏已完成：内部 `validateVideoDeliverables()` 返回可供脚本读取的本地路径，但 CLI `--json` 和可复制共享报告默认把 `artifactsDir`、`files[].path` 等本机路径替换为 redacted marker，只保留 file kinds、summary counts、warning/error code 和 verdict。
8. 更多匿名公开视频课程样例：只选公开、非登录态、可下载、包含课件画面的短样例。
