...
- P2-2A/B/C/D/F 的本地质量切片已完成：质量报告、bright-canvas detector、讲师小窗/外部前景 crop、全屏内容页 `full_frame_content_v1` detector、暗色/亮色/纯色低信息过滤、短动画转场过滤、OCR evidence 经 redaction 后进入 notes/Markdown/PPTX speaker notes、缺 OCR evidence 的低严重度风险提示、清晰度/可读性风险提示均有本地验证。
...
4. 更明确的 crop detector：针对仍可见但边界不稳定的 public 页做 fixture。
5. 清晰度/readability 分流：把低清晰、低对比和 fade 页更明确标为人工复核。
