...
| 105 | P5 static-page field candidate helper 拆分 | 本地已完成行为保持拆分并通过 P0 self-test；待提交、推送和 8 服务器验证。 | 提交后同步 GitHub，部署 8 服务器并跑 P0 code deploy 验证。 | `static_page_field_candidate_support` 模块单测、`static_page` 广义回归、`cargo check`、8 服务器 Web build、第三方报表/导出/临时文档/视频 PPT、静态页 5 路、Cloudflare fallback、prewarm observability self-test 和前端 Node 回归均通过；不改变第三方公开契约。 |
| 106 | P5 下一个行为保持小切片 | 待选定。 | #105 远端验证完成后，从 `platform-api` 或主站前端中选择下一个低风险 helper/UI 子模块，先补定向测试，再拆分。 | 定向测试、`cargo fmt --check`、`cargo check`、Web build、P0 smoke、本地或 8 服务器按发布范围验证通过；提交可单独回退；不改变第三方公开契约。 |
