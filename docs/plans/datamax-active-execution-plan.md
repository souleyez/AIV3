...
**更新时间：** 2026-06-08 10:18 CST
**当前性质：** 开发执行版；当前功能基线以已推送 GitHub 的最新 commit 和 M6AC 切片共同标注，实际代码落点以 `git log -1`、`git status` 和推送记录为准。入口是第 0.7 节，尤其是第 0.7.13-0.7.16 节的全量验收闭环、当前 HEAD 审计、下一轮任务卡和 M6AB plan-only 收口；M6AC 则把 quality matrix 的 review-required risk flag gate 扩展到真实 `slide_quality_report.risk_flags` 对象数组形态。当前公开视频样例仍应作为 `needs_manual_review` 真实样例，不强行标为 clean deliverable；本轮仍不部署 8 服务器。
**状态摘要：**
...
| 质量矩阵复核风险 gate | M6AA/M6AC 已完成：quality matrix self-test 报告暴露 8 类 review-required risk flags，并同时覆盖字符串 risk code 和真实 `slide_quality_report.risk_flags[]` 对象形态；`missing_ocr_evidence`、`single_slide_output_review_required`、`full_frame_rectangle_fallback`、`selected_slide_duplicates_removed` 等都会保持 `needs_manual_review` | 可宣称“质量矩阵不会把显式复核风险误判为 clean deliverable” | 后续新增 media-worker risk flag 时，先扩展该 gate 并复跑 no-live rollup |
