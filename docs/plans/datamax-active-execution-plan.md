...
如果后续脚本还没有 customer deliverables 参数，则 customer 样例先以 validation 回执和人工复核记录补足，随后再做脚本参数扩展；扩展前不得把 customer gate 标为完成。
...
1. 单页输出风险提示：对真实 public sample 只选出 1 页的情况，加质量报告 review risk，而不是把 `final_pptx_ready` 改成失败。
