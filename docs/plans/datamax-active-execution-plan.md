...
| 顺序 | 当前项 | 状态 | 下一步 | 关闭标准 |
| --- | --- | --- | --- | --- |
| 1 | P3 prewarm observability 回执补强 | 已完成本地验证、GitHub 同步、8 服务器 docs/script sync 和脚本级 smoke。 | 本项关闭；下一步默认继续 P5 小切片，或在拿到 P1/P2/P4 条件或新 P3 客户样例时切换优先级。 | observability receipt 暴露 `waitingForLowLoadCount`、`customerVisiblePrewarmLeakCount`、`prewarmCustomerVisibilityOk`；self-test 和 P0 smoke 通过；validation 写明生产 prewarm 仍未启用、无客户可见输出。 |
| 2 | 是否同步 8 服务器 | 本切片 docs/script 已同步；未 build、未重启，服务 active。 | 下个代码切片再按 P0 code deploy 边界执行；docs-only 或 smoke-only 继续 fast-forward。 | 8 服务器 HEAD 与 GitHub `main` 一致；服务全 `active`；`/healthz`、`/readyz` 正常；8 服务器验证结果写入 validation。 |
