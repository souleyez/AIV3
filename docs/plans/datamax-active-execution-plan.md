...
| 243 | P5 asset library scope summary assembly helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `asset_library_scope_summary_support` 新增 `load_asset_library_scope_summary_response`、`authorized_dataset_ids_for_scope_summary` 和 `asset_library_scope_summary_input_from_records`，把资产库 scope summary 的 asset library 加载、membership 加载、可见数据集过滤、scope 计算、授权资产/画像供料加载和响应输入装配从 `get_asset_library_scope_summary` 下沉；handler 仍负责用户 session、path/header 解析和 `Json` 响应包装，archived 数据集过滤、denied 计数、membership/dataset 响应字段、asset/profile hint 字段和公开契约保持不变；`cargo fmt --check`、`cargo test -q -p platform-api asset_library_scope_summary_support --lib`、`cargo test -q -p platform-api asset_library --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
| 244 | P5 dataset update support helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `dataset_update_support` 新增 `update_dataset_and_load_summary` 和 `dataset_update_fields_from_request`，把数据集更新中的可见性加载、owner 校验、title/description trim、lifecycle 解析、`archived_at` metadata、storage update 和 `DatasetSummary` 构造从 `update_dataset` 下沉；handler 仍负责路径/header/current user 解析、`Utc::now()` 和 `Json` 包装，空白 title no-op、非法 lifecycle 校验、archive metadata、权限语义、响应字段和公开契约保持不变；`cargo fmt --check`、`cargo test -q -p platform-api dataset_update_support --lib`、`cargo test -q -p platform-api asset_library_scope_summary_support --lib`、`cargo test -q -p platform-api asset_library --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
