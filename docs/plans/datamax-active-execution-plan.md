...
- M6S 已完成：新增 `smoke:video-ppt-no-live-rollup`，把视频 PPT 无授权 self-test/preflight/check/test 固化成 15 命令一键回归，报告安全 gate 明确 `live_smoke_run=false`、`network_download_allowed=false`、`file_upload_allowed=false`、`browser_capture_allowed=false`、`service_deployment_allowed=false`。
...
6. no-live rollup 已完成：`npm run smoke:video-ppt-no-live-rollup -- --self-test` 一次性执行主站上传、第三方、handoff、授权录屏、quality matrix 和 deliverables validator 的无授权门禁，并输出脱敏报告。
...
| M6S | video PPT no-live rollup gate | 无 live 授权；只执行 self-test/preflight/check/test | 已通过 `node --check scripts/smoke/video-ppt-no-live-rollup.mjs`、`npm run smoke:video-ppt-no-live-rollup -- --self-test --pretty --output-dir <target-redacted>`、report redaction scan | 已完成：15 个无授权命令一键通过，覆盖主站上传、第三方登记、视频号 handoff、授权录屏 helper、quality matrix 和 deliverables validator；报告安全 gate 明确未跑 live、未下载、未上传、未录屏、未部署、未触碰 8/120 服务器 | 未部署 |
