...
| 248 | P5 dataset output list support helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `dataset_output_list_support` 新增 `list_dataset_output_views_for_user` 和 output owner filter，把 `list_dataset_outputs` 中的数据集可见性校验、dataset output 加载、owner 过滤和 view hydration 下沉；handler 仍负责 path/header/current user 解析和 `Json` 包装，dataset scope 权限、匿名/登录 owner 可见性、输出排序、hydrated `DatasetOutputView` 字段和公开契约保持不变；`cargo fmt --check`、`cargo test -q -p platform-api dataset_output_list_support --lib`、`cargo test -q -p platform-api dataset_output --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
| 249 | P5 dataset output retrieval evidence support helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `dataset_output_evidence_support` 新增 `list_dataset_output_retrieval_evidence_views_for_user`，把 `list_dataset_output_retrieval_evidences` 中的 dataset output 可见性加载、retrieval evidence 按 id 加载、可见文档过滤和 view 转换下沉；handler 仍负责 output id/path、active secret header、current user 解析和 `Json` 包装，输出权限、证据过滤、证据顺序、`RetrievalEvidenceView` 字段和公开契约保持不变；`cargo fmt --check`、`cargo test -q -p platform-api dataset_output_evidence_support --lib`、`cargo test -q -p platform-api dataset_output --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
| 250 | P5 dataset retrieval evidence support helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `dataset_retrieval_evidence_support` 新增 `list_dataset_retrieval_evidence_views_for_user`，把 `list_dataset_retrieval_evidences` 中的数据集可见性校验、latest retrieval evidence 加载、可见文档过滤、相关性排序和 view 转换下沉；handler 仍负责 dataset id/path、active secret header、current user 解析和 `Json` 包装，100 条扫描上限、证据可见性、排序规则、`RetrievalEvidenceView` 字段和公开契约保持不变；`cargo fmt --check`、`cargo test -q -p platform-api dataset_retrieval_evidence_support --lib`、`cargo test -q -p platform-api dataset_output --lib`、`cargo test -q -p platform-api retrieval_evidence --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
| 251 | P5 chat session list support helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `chat_session_list_support` 新增 `list_chat_session_views_for_user` 和 session owner filter，把 `list_chat_sessions` 中的数据集可见性校验、chat session 加载、owner 过滤和 view hydration 下沉；handler 仍负责 dataset id/path、active secret header、current user 解析和 `Json` 包装，会话权限、列表顺序、latest message/output/memory hydration、model_facing 和公开契约保持不变；`cargo fmt --check`、`cargo test -q -p platform-api chat_session_list_support --lib`、`cargo test -q -p platform-api chat_session --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
| 252 | P5 memory directory list support helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `memory_directory_list_support` 新增 `list_memory_directory_views_for_user`，把 `list_memory_directories` 中的数据集可见性校验、memory directory 加载、可见文档 scope 过滤和 view 转换下沉；handler 仍负责 dataset id/path、active secret header、current user 解析和 `Json` 包装，目录权限、列表顺序、typed directory tree 和公开契约保持不变；`cargo fmt --check`、`cargo test -q -p platform-api memory_directory_list_support --lib`、`cargo test -q -p platform-api memory_directory --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
| 253 | P5 document chunk list support helper 拆分 | 本地完成。 | 后续继续 P5 行为保持切片，或在用户要求发版时与 P0 一起部署验证。 | `document_chunk_list_support` 新增 `list_document_chunk_views_for_user`，把 `list_document_chunks` 中的文档可见性校验、document chunk 加载和 view 转换下沉；handler 仍负责 document id/path、active secret header、current user、local thread header 解析和 `Json` 包装，local-only 文档 scope、chunk 顺序、typed chunk state/section hints 和公开契约保持不变；`cargo fmt --check`、`cargo test -q -p platform-api document_chunk_list_support --lib`、`cargo test -q -p platform-api document_chunk --lib`、`cargo check -q -p platform-api` 通过；不改变第三方公开契约。 |
...
1. 默认继续 #254：选择下一个 `platform-api` 或主站前端行为保持小切片，先补定向测试，再做拆分和 P0 回归。
