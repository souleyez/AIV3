...
| 顺序 | 当前项 | 状态 | 下一步 | 关闭标准 |
| --- | --- | --- | --- | --- |
| 1 | P5 external bot message payload normalization helper 拆分 | 本地已完成行为保持拆分和 P0 回归。 | 提交 GitHub 后按 code deploy 边界同步 8 服务器，跑远端定向回归和 P0 smoke。 | `external_bot_message_payload_support` 模块单测、第三方 payload alias 旧回归、`external_channel_static_page_artifact`、`cargo check`、Web build、P0 smoke、8 服务器 release build、health/ready 和服务 active 全通过；不改变第三方公开契约。 |
| 2 | 是否同步 8 服务器 | 等待本轮代码提交后同步。 | code deploy 后写回远端验证；docs-only 回写不重启服务。 | 8 服务器 HEAD 与 GitHub `main` 一致；服务全 `active`；`/healthz`、`/readyz` 正常；8 服务器验证结果写入 validation。 |
