...
| 顺序 | 当前项 | 状态 | 下一步 | 关闭标准 |
| --- | --- | --- | --- | --- |
| 1 | P5 document chunk/enrichment run view helper 拆分 | 已完成本地验证；待提交、GitHub 同步和 8 服务器发布判断。 | 提交 `document_view_support` 切片，发布时拉取最新 `main`、build `platform-api`、重启 DataMax 相关服务、跑 8 服务器 P0 smoke。 | `cargo test -p platform-api document_view_support --lib`、document chunk/enrichment run view mapping 单测、`cargo fmt --check`、`cargo check -p platform-api`、Web build、P0 smoke 全通过；validation 写明未改公开接口/鉴权/字段；提交可单独回退。 |
...
**Current progress:**
- 2026-06-14: `crates/platform-api/src/lib.rs` document chunk 和 document enrichment run view mapping helper 已拆到 `document_view_support` 模块；本地 `cargo fmt --check`、`cargo test -p platform-api document_view_support --lib`、`cargo test -p platform-api to_document_chunk_view_exposes_typed_state --lib`、`cargo check -p platform-api`、第三方静态页触发单测、Web build 和 P0 smoke 均通过，待提交和发布判断。
...
1. 先关闭当前 P5 `document_view_support` 切片：提交、同步 GitHub、按 P0 code deploy 边界发布 8 服务器并回写 validation。
