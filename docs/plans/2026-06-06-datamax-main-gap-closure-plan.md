...
| P1 | G6 data-source row identity review | Read-only review completed for current `hy-sql-traffic-area` smoke sync. Collapse is intentional under current mapping for `bi_contract_warning` (`parentcode`) and `bi_rentsales_detail` (`storecode`) but not row-level complete. | If row-level materialization is required, change identity mapping in a staging-only slice, e.g. use the configured composite `id_columns`, then rerun guarded sync and compare counts. | Operators can distinguish source rows, unique documents, chunks, evidence, and collapsed rows; Xinbai database-derived reports are not misleading about row-level completeness. |
