...
- Local `main` was clean at `279c343867ac` before the P1 follow-up documentation update.
- 8 server `/srv/aiv3/repo` was synced through `279c343867ac` after the P0 rollout; service code for the latest prewarm path is deployed from `5c267abff53d`; `aiv3-platform-api.service`, `aiv3-web.service`, `aiv3-static-page-worker.service`, and `aiv3-document-enrichment-worker.service` are active.
...
| --- | --- | --- | --- | --- |
| P0 | G3 low-load static-page template prewarm | Implemented and deployed at `5c267abff53d`. Silent prewarm creates a customer-invisible draft, delayed existing `static_page/generate_static_page_image` task, low-load worker recheck, Image2 preview, then local render/publish. Production flag remains unset by default. | Keep production default off until an operator decides to enable background warming. If enabled, use low-load thresholds and monitor queue pressure. | Passed 8-server private smoke: normal answer unaffected, no customer artifact event, no outbound reply, no Codex task in the new local-publish path, accepted template stored, repeated same scope/default_prompt skipped. |
| P0 | Release smoke after any P0 change | 8 server service code is deployed at `5c267abff53d`; docs-only syncs may move the repo forward without rebuilding services. Existing untracked `mode` remains untouched. | Watch the queue after rollout and keep Codex fallback as bounded fallback only. | Validation doc records commit, service status, smoke receipts, and rollback note. Third-party public URLs/auth/request/response fields remain unchanged. |
| P1 | G5 passive low-quality recovery | Safe-disabled state reconfirmed on 8 server: hard gate unset, autofix flag unset, Codex task allowlist set but excluding `answer_quality_autofix`. | Keep disabled by default. If enabled later, enable passive collection plus manual review only, with strict fixed-task write scope and no answer suppression. | Weak-answer cases are classified and visible to operators; no normal answer is blocked; no Codex task can be created unless the dedicated flag and both allowlists are explicitly enabled. |
| P1 | G8 authenticated model-gateway operator smoke | Reusable `npm run smoke:model-gateway-operator` is implemented and deployed. It proves unauthenticated `401 auth_session_required`, supports real cookie or email+local-key login, redacts secrets, and records pending when credentials are missing. No legitimate operator credential was available in this thread, so authenticated status/profile/test remains pending. | Run the new smoke with an existing legitimate operator cookie or local-key login; add `--run-profile-test` only when a real provider probe is approved. | Operator page can show model lane health and profile state for a signed-in operator; unauthenticated access still returns `401`; report contains no cookie, local key, provider secret, or raw auth env name. |
| P1 | G6 data-source row identity review | Reusable read-only identity audit is now part of `scripts/run-data-ingestion-staging-live-smoke.sh` and passed on 8 server. It shows two latest-sync tables still collapse source rows even with composite `id_columns`: `bi_contract_warning` and `bi_rentsales_detail`. | If row-level materialization is required, test a staging-only mapping with additional row discriminator columns for those fact/detail tables, then compare source rows, unique documents, chunks, evidence, and report behavior before production adoption. | Operators can distinguish source rows, unique documents, chunks, evidence, and collapsed rows; Xinbai database-derived reports are not misleading about row-level completeness. |
