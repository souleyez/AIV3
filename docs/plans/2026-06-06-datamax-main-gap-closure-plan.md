...
| P1 | Data-source row identity | Current-head `15756668bcad` 8-server live audit passed and confirms question/report readiness, but `bi_contract_warning` and `bi_rentsales_detail` collapse 193 latest-sync rows under current identity. | Decide entity-level vs row-level semantics; if row-level is required, test staging-only discriminator mapping. | Report completeness semantics are documented and validated before production mapping changes. |
