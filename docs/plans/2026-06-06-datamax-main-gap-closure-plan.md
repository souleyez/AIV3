...
| P0 | Release smoke after any P0 change | 8 server is deployed at `5c267abff53d`; existing untracked `mode` untouched. | Watch the queue after rollout and keep Codex fallback as bounded fallback only. | Validation doc records commit, service status, smoke receipts, and rollback note. Third-party public URLs/auth/request/response fields remain unchanged. |
