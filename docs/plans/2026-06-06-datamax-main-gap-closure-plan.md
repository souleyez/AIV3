...
- Still open:
  - immediate deployed-head closeout: 8 server is now at `44f653ff6001`, `aiv3-platform-api.service` and `aiv3-codex-host-agent.service` are active, and the known untracked `mode` file remains untouched; the focused report/export and capability-routing smoke still need to be rerun with the active `local-dev` `generic-chat-main` token because an unqualified duplicate-channel token lookup produced HTTP `401`;
...
| --- | --- | --- | --- | --- |
| 1 | Deployed `44f653ff6001` smoke closeout | The Xinbai primary-template patch is on 8 server, but deployed evidence is not yet recorded. | Rerun report/export smoke and selected capability-routing smoke with the active `local-dev` channel token, then update `docs/validation/datamax-main-gap-closure.md` and `docs/validation/static-page-render-smoke.md`. | JSON/SSE report export pass; one clickable report link; export files reachable; focus is correct; selected routing fixtures pass; no token or raw customer data is recorded. |
...
| 5 | Data-source row identity | `bi_contract_warning` and `bi_rentsales_detail` still collapse latest-sync rows under current identity. | Decide business semantics: entity/latest-snapshot is acceptable, or row-level detail is required. If row-level is required, validate discriminator mapping in staging only. | Production mapping stays unchanged until staging proves counts, chunks, evidence, and reports are correct. |
| 6 | Historical enrichment/backfill | New documents enrich, but historical corpora are not fully backfilled. | Keep `--summary-only --dry-run` as default. Only after review, run one small real batch for one dataset and one enrichment kind. | No titles/content/URLs/secrets printed; no duplicate fact inflation; production backfill remains disabled unless approved. |
...
npm run smoke:external-report-export -- --base-url https://v3.elepcloud.com --connection-id generic-chat-main --timeout-ms 180000 --output-dir target/external-report-export-smoke-task8-44f653f-localdev
```
...
| --- | --- | --- | --- | --- |
| P0 | Final 8-server release gate | 8 server is at `44f653ff6001`; `aiv3-platform-api.service` and `aiv3-codex-host-agent.service` are active; known `?? mode` remains untouched. Prior release-gate smokes passed on earlier current heads, but the latest template-hygiene patch needs focused deployed smoke. Main-site chat-session 20-way remains auth/scope-dependent and Cloudflare/model-gateway guard remains pending without operator credentials. | Run the focused report/export and capability-routing smoke for `44f653ff6001` with the active `local-dev` channel token, then record results. Separately provide a legitimate main-system session/operator credential for auth-dependent gates. | Latest deployed commit has smoke receipts; service status and rollback note are recorded; auth-dependent checks pass or are explicitly pending with no bypass. |
