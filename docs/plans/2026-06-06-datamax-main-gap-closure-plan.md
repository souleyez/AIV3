...
| 6 | Historical enrichment/backfill | New documents enrich, but historical corpora are not fully backfilled. Current-head summary-only dry-runs, including one dataset-scoped run, remain safe and wrote no records. | Keep `--summary-only --dry-run` as default. Only after review, run one small real batch for one dataset; enrichment-kind filtering is not currently supported by the backfill tool. | No titles/content/URLs/secrets printed; no duplicate fact inflation; production backfill remains disabled unless approved. |
...
| P1 | Historical enrichment/backfill | New documents enrich; full historical backfill disabled. 8-server current-head `112cc82e8457` `--summary-only --dry-run` passed with `candidate_count=20`, `skipped_count=20`, `would_record_count=0`, `recorded_count=0`. A one-dataset `--summary-only --dry-run` for dataset `cd024465-358e-458c-961d-a8894f2358c5` also passed with `candidate_count=20`, `recorded_count=0`, `would_record_count=0`. | Keep production backfill disabled until reviewed. If approved, run only one tiny real batch for one reviewed dataset; note the current backfill tool supports dataset/document filters but not enrichment-kind filters. | No document titles/content printed; no production backfill until reviewed. |
...
6. **P1 historical enrichment:** keep summary-only dry-run as the default; only run a tiny real batch after reviewing dataset/document scope, queue load, and rollback behavior. The current backfill tool supports dataset/document filters, not enrichment-kind filters.
