...
| 6 | Historical enrichment/backfill | New documents enrich, but historical corpora are not fully backfilled. Current-head fingerprint summary-only dry-runs, including one dataset-scoped run, remain safe and wrote no records. `fact-index-backfill` now has a local summary-only path and real-run guard pending 8-server rollout. | Keep `--summary-only --dry-run` as default. Only after review, run one small real batch for one dataset; fact-index real runs must use `--confirm-real-run`, and dataset-level real runs must use an explicit `--limit <= 5`. | No titles/content/URLs/secrets printed; no duplicate fact inflation; production backfill remains disabled unless approved. |
...
| P1 | Historical enrichment/backfill | New documents enrich; full historical backfill disabled. 8-server current-head `112cc82e8457` fingerprint `--summary-only --dry-run` passed with `candidate_count=20`, `skipped_count=20`, `would_record_count=0`, `recorded_count=0`. A one-dataset fingerprint `--summary-only --dry-run` for dataset `cd024465-358e-458c-961d-a8894f2358c5` also passed with `candidate_count=20`, `recorded_count=0`, `would_record_count=0`. Local `fact-index-backfill` now supports `--summary-only` and rejects real writes unless `--confirm-real-run` is present; dataset-level real writes also require an explicit `--limit <= 5`. | Keep production backfill disabled until reviewed. If approved, run only one tiny real batch for one reviewed dataset; note fingerprint/fact-index tools support dataset/document filters, while the enrichment worker supports kind/max-run controls only for already queued runs. | No document titles/content printed in summary-only mode; no production backfill until reviewed. |
