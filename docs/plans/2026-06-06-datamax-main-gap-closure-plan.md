...
| P1 | Authenticated model-gateway operator smoke | Current-head 8-server unauthenticated guard passes with `401 auth_session_required`; no operator smoke cookie/email/local-key is configured in checked env files. | Run `smoke:model-gateway-operator` with a legitimate operator session or local-key login. | Operator status/profile health is proven without bypasses or leaked secrets. |
