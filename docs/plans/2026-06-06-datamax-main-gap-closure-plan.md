...
| P1 | G9 release hygiene | Commits through the post-ingest rollout are pushed. 8 server is on latest `main`; `aiv3-retrieval-worker.service` was rebuilt/restarted for the latest G4 slice, and `aiv3-document-enrichment-worker.service` is active. | Keep every future deploy tied to a commit, service status, smoke receipt, and rollback note. | 8 server is on latest `main`, all changed services are active, and validation docs match reality. |
...
13. Next: decide whether to explicitly enable `answer_quality_autofix` live enqueue for passive collection plus manual review only; separately decide whether low-load static-page template prewarm should be enabled now that controlled streaming is stable.
