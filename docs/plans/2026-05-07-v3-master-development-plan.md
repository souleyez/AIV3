...
17. Partially completed current slice: surface raw frame extraction `failed` / `skipped` states, generated-artifact writer failures, parse-partial evidence, and low-confidence transcript/OCR evidence as structured deliverable warnings with reason/count/missing-evidence details, and map those plus core review warnings into completion follow-up next actions. Remaining failure handling: unsupported source, unavailable video, resolver blocked, provider failure, and no slide rectangle found.
