...
8. Partially completed current slice: `media-worker` emits a disabled-by-default FFmpeg/raw_frames extraction plan using `video-extraction-{document_id}/raw_frames`, `frame_%06d.jpg`, interval `0.15`, and contact-sheet source `raw_frames`, matching the `wechat-video-ppt-extract` SOP. Remaining work is executing FFmpeg in the worker and persisting the frame manifest.
