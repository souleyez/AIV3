...
**2026-05-16 immediate UI follow-up:** AssistantRun create/continue responses now include the same safe diagnostics as the detail API, and the existing chat progress card renders a compact Codex promotion checklist from `promotion_gate.readiness_checks`. The progress parser is now separately tested, also summarizes safe `provider_usage` metadata, falls back to recent provider events when summary counts are sparse, shows promotion `blocked_by` / `next_step` details, and surfaces fixed-field context-budget/tool-output trimming counts without rendering raw structured blocker payloads or raw context items. This keeps the operator signal in the current conversation surface instead of adding a new global panel.
