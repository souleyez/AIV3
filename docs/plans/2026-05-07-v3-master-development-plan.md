...
16. Partially completed current slice: completion follow-up now carries a status-only `user_notification` payload that can be surfaced by the UI without host-composing an assistant answer, while the full model-authored completion turn remains pending.
