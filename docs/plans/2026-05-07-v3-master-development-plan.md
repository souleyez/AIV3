...
Current template registry follow-up: the safe artifact allowlist now includes `third_party_handoff_document` with source type `external_integration`, and the third-party handoff package emits `html-artifacts/third-party-handoff-document.json`, giving handoff/API guides a V3-owned template slot plus a concrete package artifact before any local Codex skill is introduced.
