...
- Current V3 awareness/search slice: startup briefing now carries an additive model-awareness policy; backend AssistantRun provider input/continue input always includes V3 identity, V3 capability scope, unavailable-evidence wording (`当前不可见/未供料`), and the rule that external/web search cannot be claimed without audited V3 search evidence; external channel AssistantRun startup briefing also persists the same model-awareness and visible-scope policies for observability/Codex context reuse; Codex action contracts now expose a read-only `web_search` request that requires V3 validation and search evidence with source/time metadata before citation; external-channel `web_search` suggestions now return `v3_search_evidence_required` and redacted audit events instead of creating third-party action runs or claiming live search; external integration audit can filter those rows with `item_type=search_evidence`, and the standalone panel exposes a `搜索证据` filter.
