...
15. Partially completed current slice: generated `source_text.md` and `ppt_outline.md` now include transcript/scene/OCR evidence refs, confidence/source labels, explicit quality notes for missing/low-confidence evidence, provider capability failures, frame-extraction skips/failures, and redacted provider capability status; richer transcript segmentation, keyframe clustering, OCR slide text promotion, and PPTX rendering remain pending.
16. Partially completed current slice: completion follow-up now carries a status-only `user_notification` payload, and the safe video summary plus right-shelf card consume it without host-composing an assistant answer; the full model-authored completion turn remains pending.
