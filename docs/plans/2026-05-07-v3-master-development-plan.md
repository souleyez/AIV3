...
17. Partially completed current slice: surface raw frame extraction `failed` / `skipped` states, generated-artifact writer failures, parse-partial evidence, provider failures, and low-confidence transcript/OCR evidence as structured deliverable warnings with reason/count/missing-evidence/provider details; classify resolver failures for unsupported source, unavailable video, resolver blocked, and missing source; and map those plus core review warnings into completion follow-up next actions. Remaining failure handling: no slide rectangle found.
