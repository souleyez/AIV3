...
7. Partially completed current slice: assistant extraction requests now queue `video_extraction_workflow` for selected uploaded/resolved video documents when parsed evidence is missing. Remaining work is to wire the actual worker steps to the existing media parse path and final artifact writer.
