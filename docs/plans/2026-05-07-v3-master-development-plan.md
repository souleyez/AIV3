...
17. Partially completed current slice: surface raw frame extraction `failed` / `skipped` states as structured deliverable warnings with reason text. Remaining failure handling: unsupported source, unavailable video, resolver blocked, parse partial, provider failure, no slide rectangle found, subtitle OCR low confidence, and low-confidence extraction.
