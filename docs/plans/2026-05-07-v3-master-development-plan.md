...
Current evidence-manifest follow-up: the third-party handoff package now writes a final `<package>.evidence-manifest.json` after aggregate evidence, and `validate:evidence` verifies every final delivery artifact plus `.all.json`/`.all.md` without introducing a circular delivery-manifest hash dependency. The final evidence manifest gate also locks the package type to `v3.external_third_party_handoff_package.v1`, writes `<package>.evidence-manifest.md`, and supports `validate:evidence --markdown <file>.md` for a human-readable final evidence receipt.
