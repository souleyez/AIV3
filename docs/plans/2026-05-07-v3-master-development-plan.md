# DataMax Master Development Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Provide the single active development plan for DataMax, with static-page generation, parsing/retrieval quality, ordinary AssistantRun reliability, and DataMax-owned model-gateway/provider routing as the current product mainline. The Codex substrate remains documented but is frozen as of 2026-05-18 until the operator explicitly resumes it.

**Architecture:** DataMax remains the control plane and source of truth: PostgreSQL owns identity, dataset visibility, AssistantRun state, memory scope, workflow state, and artifacts. The model gateway owns provider profiles and routing across GPT, MiniMax, and other OpenAI-compatible or adapted model APIs; the current production answer path stays on provider routing. Static-page generation, reports, parsing, retrieval, and media understanding stay as DataMax product capabilities. The previously planned Codex conversation executor / Codex Host substrate remains a dormant future execution-host option, not the current model gateway or authoritative answer path.
...
2026-05-18 update: the Codex substrate track is frozen. 8 server has Codex CLI installed but not logged in; online DataMax AssistantRun still uses the MiniMax/provider path; Codex Host remains plan-only with real execution disabled. Continue backend/static-page/retrieval/ordinary-chat quality work without promoting Codex until the operator explicitly resumes that track.
...
2026-05-25 plan consolidation: parsing quality, answer quality, queryable facts, Codex executor boundaries, data-ingestion analysis, and static-page productization now have one active plan at `docs/plans/2026-05-25-DataMax-mainline-quality-executor-plan.md`. Keep this master document as historical context and high-level feature background. Do not change third-party public interfaces without an explicit operator decision.
...
- `docs/plans/2026-04-29-DataMax-consolidated-development-handoff-plan.md`: older consolidated handoff and status log.
...
- `docs/plans/2026-05-07-DataMax-codex-host-architecture-alignment-plan.md`: detailed module-boundary plan for Codex Host.
- `docs/plans/2026-05-07-DataMax-email-account-auth-plan.md`: detailed account/email auth plan; account work is paused after current hardening unless explicitly resumed.
- `docs/plans/2026-05-20-DataMax-context-document-understanding-p0.md`: detailed third-party document scope, async parse, parse-quality, direct-reply, user-context, and document-understanding P0 plan.
...
- `docs/plans/2026-05-25-DataMax-mainline-quality-executor-plan.md`: active consolidated mainline plan for parser quality, document understanding, retrieval/fact supply, answer reliability, static-page generation, data-ingestion analysis, and Codex executor boundaries.

When old documents conflict with this one, follow this master plan.

## Product North Star

DataMax is not a generic file manager and not a standalone page builder. It is a data-aware intelligent assistant that can:
...
- Let the model choose retrieval/tool actions through DataMax validation.
- Serve assistant conversations through DataMax-owned provider routing and DataMax-supplied context, evidence, memory, tool contracts, and safety gates. The Codex execution-kernel route is frozen and should not be promoted without an explicit operator resume decision.
...
- Ingest read-only external database sources into explicit DataMax datasets through DataMax-owned source connectors, mapping rules, workflow sync, parsing, indexing, and dataset visibility checks, then reuse the normal dataset question-answering, report, template, and static-page chains rather than direct chat-time database access.
...
- No local answer composition. DataMax supplies context and actions; the model writes the answer.
- DataMax model awareness is global and additive, not restrictive. Every AssistantRun surface must brief the model that it is serving through DataMax, what DataMax can do, what permission-filtered datasets/tools are visible, and that the model may still answer general questions outside DataMax when useful.
- If DataMax has not supplied permission, evidence, or tool results for a topic, the answer must say the relevant DataMax scope is currently not visible or not supplied (`当前不可见/未供料`) before continuing with clearly labeled general model knowledge or judgment.
- External/web search should become a default DataMax-controlled read-only capability. Until live search evidence is actually supplied, no answer may imply that DataMax searched the web; when implemented, search results must enter context as audited evidence with source and timestamp metadata.
- Codex may only participate in dormant shadow/plan-only diagnostics while this track is frozen. DataMax remains the authority for permissions, visible datasets, memory scope, workflow state, queue submission, model routing, and artifact ownership.
...
- HTML artifacts must be sandboxed DataMax-owned render templates. Do not display raw provider/model HTML with scripts, remote assets, secrets, queue credentials, or direct database actions.
...
- External bot and third-party integrations are surfaces, not new authorities. DataMax must still own effective permissions, AssistantRun state, retrieval supply, artifact access, action validation, and audit.
- Database sources are external-source inputs, not direct model tools. Every database sync must resolve to a DataMax dataset before rows can affect answers or reports. Store only redacted connection metadata and server-side environment variable names in DataMax; never store raw database URLs, passwords, tokens, or arbitrary SQL in DataMax PostgreSQL, logs, browser state, workflow events, assistant messages, or repository files.
...
- Static-page intent no longer interrupts ordinary conversation. When the user asks for a static page in chat, DataMax still lets the assistant answer normally and appends a draft entry card at the end of the chat with `进入静态页工作台`; the main workspace switches to module editing only after the user clicks that entry.
...
- Current DataMax awareness/search slice: startup briefing now carries an additive model-awareness policy and explicit supply evidence boundaries that separate citable supplied items/observations from planning-only candidates, dataset briefs, startup briefing, and detail targets; backend AssistantRun provider input/continue input always includes DataMax identity, DataMax capability scope, unavailable-evidence wording (`当前不可见/未供料`), and the rule that external/web search cannot be claimed without audited DataMax search evidence; normal Host-Controlled ReAct now accepts a read-only `web_search` action that returns a DataMax search-evidence-required observation without leaking raw query text; external channel AssistantRun startup briefing also persists the same model-awareness and visible-scope policies for observability/Codex context reuse; Codex action contracts now expose a read-only `web_search` request that requires DataMax validation and search evidence with source/time metadata before citation; external-channel `web_search` suggestions now return `v3_search_evidence_required` and redacted audit events instead of creating third-party action runs or claiming live search; external integration audit can filter those rows with `item_type=search_evidence`, and `GET /v1/external/integrations` plus the standalone panel expose channel-level search-evidence pending summaries.
...
- Java 8 parity audit is complete: Java/Vue used `gridstack` plus `echarts`; DataMax keeps `react-grid-layout` and adds ECharts as advanced chart runtime.
...
- The next architecture uplift is not Codex promotion. Keep strengthening DataMax-owned provider routing, ordinary AssistantRun quality, retrieval/context supply, and static-page generation quality.
...
Also owns safe artifact viewers for static-page handoff previews, Codex execution reports, code review/project inventory summaries, and lightweight HTML editors that emit JSON patch requests back to DataMax.
...
It is not a browser API, model gateway, memory authority, permission authority, or DataMax scheduler. For normal chat, the current path remains DataMax provider routing, not Codex Host.
...
Also owns durable HTML artifact records, manifests, provenance, versioning, and export packages. HTML artifacts are products of trusted DataMax templates plus sanitized data, not arbitrary browser documents.
...
The DataMax-owned model gateway remains active. The Codex conversation executor part of the previous track is frozen as of 2026-05-18.

The active target flow is:

1. Browser sends a conversation turn to DataMax.
2. DataMax creates or continues an `AssistantRun`.
3. DataMax builds a supply package: system/product briefing, visible datasets, selected/inferred scope, hidden-memory candidates, evidence, current artifact state, tool catalog, and safety policy.
4. DataMax sends the prompt and supplied evidence to the configured provider path through `llm-gateway` / AssistantRun runtime selection.
5. The model answers or requests DataMax-validated ReAct actions through the existing provider path.
6. DataMax validates every requested action against user/session/dataset/artifact scope.
7. DataMax persists events, supplied evidence, actions, artifacts, and the final model-authored response.

For static-page turns, Codex action planning must respect DataMax's data-quality gates. A request for an effect image is not enough by itself: if the current artifact reports missing bindings, matched-field-only candidates, inferred chart signals, or chart data without sample rows, the executor should first request retrieval or module repair through DataMax-validated actions. This keeps the GPT Image bridge and final renderer from turning weak evidence into polished-looking fake dashboards.
...
- A DataMax-owned private Responses-compatible shim pattern for providers Codex cannot call natively, using `CoDeepSeedeX` as a design reference but not as a runtime dependency.
- Provider-shim observability: local-only health/status, capability, balance/usage, trace, and context-budget diagnostics. Persist product-grade usage/audit into DataMax PostgreSQL/runtime inspect rather than relying on shim-local SQLite as source of truth.
...
- DataMax-supplied context instead of Codex directly reading databases or local files.
...
This track must not disrupt the current static-page user flow. DataMax keeps the existing static-page state machine, draft model, preview queue, final renderer, artifact ownership, and current UI progression.
...
DataMax must gain an external integration mainline without becoming a platform-specific chatbot fork.
...
- Third-party document parsing through the existing DataMax ingest/retrieval workers, with source document ids, revisions, provenance, and ACL snapshots preserved.
- Effective permission resolution from external user identity, source users/departments/groups/roles, source document ACLs, DataMax tenant/account policy, dataset visibility, and channel policy.
...
- DataMax-validated external transactions with explicit risk levels, confirmation requirements, audit records, and redacted failure summaries.
- An observe-first DataMax management UI for connection health, sync status, permission drift, message/run traces, retrieval quality, transaction status, and retry/disable/resync/secret-rotation controls.

Do not add a second DataMax chat surface for this track. External chat pages may live outside DataMax, while DataMax's own UI stays focused on observability and governance.
...
Current skillization decision: `docs/architecture/html-artifact-skill-evaluation.md` records the decision to turn this into a reusable DataMax safe-HTML artifact capability first, with a possible local Codex skill later. The external `dogum/html-artifacts` project is a useful Apache-2.0 reference for recognition heuristics, but DataMax should not vendor or install it directly until the product template rules and license review are settled.
...
DataMax should use this layer for:
...
- Lightweight temporary editors that collect edits and send JSON patch operations back to DataMax.
...
- AssistantRun/model track: model gateway profiles, provider quality, DataMax context supply packages, validated action loop, and concise progress reporting.
...
The product track must remain safe without real local host execution. DataMax still answers product questions through model-authored provider output, retrieves evidence, generates reports/static pages, and manages artifacts through its own API/worker/data planes.

The next highest leverage order is:

1. Preserve the current overall UI shell and stop broad shell redesign.
2. Stabilize the third-party close loop: document parse into DataMax datasets/scopes, conversation-scoped document ranges, requested skills, template skill output, selected output format, parser status supply, and model-authored direct replies. The old fixed "已收到指令..." style fallback must not be the normal failure mode.
3. Improve parsing and answer quality through the consolidated mainline plan: async parse lifecycle, parse-quality detection, PaddleOCR structured parse, MiniMax VLM fallback, paragraph/section segmentation, heading hierarchy, table structure, entities, noun terms, queryable facts, retrieval/evidence quality, and direct model-authored answers are now governed by `docs/plans/2026-05-25-DataMax-mainline-quality-executor-plan.md`.
...
5. Bring database sources through the same dataset chain in P0 managed mode: DataMax operators configure the database source, bind a dataset, inspect/profile/sync rows, and third-party conversations consume only selected/visible dataset evidence and aggregates.
...
The main architectural risk is letting too many "brains/builders" compete: direct model calls, frozen Codex executor designs, static-page renderer, and HTML artifact renderer. While Codex is frozen, the boundary is simpler: DataMax provider routing handles model-authored answers/actions, DataMax validates and supplies data, static-page renderer produces customer report pages, and HTML artifact renderer displays review/control artifacts.

## Immediate Execution Track

The next development threads should stay split:

- **Active mainline thread:** run `docs/plans/2026-05-25-DataMax-mainline-quality-executor-plan.md` only. It owns parsing quality, PaddleOCR/MiniMax fallback, document structure/entity/term extraction, queryable fact supply, retrieval/evidence quality, answer reliability, static-page generation, data-ingestion analysis, and Codex executor boundaries.
...
- Create: `docs/plans/2026-05-07-DataMax-master-development-plan.md`
...
4. Commit with `docs: add DataMax master development plan`.
...
**2026-05-16 suggested-action UI follow-up:** The chat progress card now also surfaces the safe Codex `suggested_action` summary: action type, title/source, confidence, DataMax-validation requirement, mutation/queue guard state, confirmation requirement, argument-presence flag, and input-schema presence. Raw suggested-action arguments stay hidden, so prompt text, provider keys, and action payload details are not rendered in the browser.

**2026-05-16 host-invocation UI follow-up:** The chat progress card now surfaces the safe `host_invocation` boundary summary: invocation kind, requested transport, host-required flag, local-execution/mutation/queue guards, input contract, output schema title/required-count, and DataMax/direct-access safety booleans. The frontend intentionally ignores `command_blueprint` args/stdin/workspace plus model-gateway profile/env details so a future host launch plan cannot leak prompts, paths, secrets, or local command arguments through the browser.
...
- Boundary decision: DataMax may implement its own private local Responses-compatible shim for MiniMax/DeepSeek-like providers, but the shim only normalizes model API traffic. It must not become the DataMax tool executor, permission authority, memory store, queue owner, or product audit source.
...
3. Define an `AssistantRunCodexContextPackage` contract containing system/product briefing, selected/inferred datasets, evidence state, hidden-memory candidates, current artifact state, available DataMax actions, and safety policy.
...
8. Ensure Codex can only request DataMax action contracts such as retrieval/detail refresh, conversation-memory recall, static-page draft creation, module update, image preview submit, final render request, and HTML artifact event submit.
...
12. Do not fork or vendor `openai/codex` in DataMax unless a concrete upstream gap blocks the supported CLI/SDK/server surfaces. If a patch is needed, keep it as a documented upstream-compatible patch set.
...
14. Store production usage/audit in DataMax tables and runtime inspect events. Shim-local ledgers may exist only as host diagnostics and must not become the source of truth.
...
17. Add tool-call protocol hardening tests for malformed provider responses, missing tool outputs, duplicate function-call replay, and liveness stalls where Codex intended to call a DataMax action but returned an incomplete tool request.
...
- Codex receives only DataMax-approved context and tool/action contracts, not unrestricted database/filesystem access.
- Multi-provider model routing is controlled by gateway profiles, not hard-coded per endpoint.
- Codex transport choice remains explicit, auditable, and dormant: `exec_schema` for one-shot tasks, `sdk_thread` or `app_server` for continuing conversations, `mcp_server` only if DataMax needs to expose Codex as a tool after the freeze is lifted.
- Non-native model providers can be attached through a private Responses-compatible shim without giving the shim direct DataMax database, queue, filesystem, or tool-execution authority.
...
- Selected/inferred dataset supply still flows through DataMax visibility checks before reaching Codex.
...
- The model knows DataMax can create reports/static pages.
...
- Required assistant behavior: the assistant must know this capability from the startup briefing/action catalog. When the user uploads a video or provides a direct/publicly resolvable video URL/page and asks to extract PPT/original text, the model should answer in its own words and request the DataMax action. If the model/provider fails, DataMax must show failure and must not compose a fake assistant answer.

**Implementation reference:** The local Codex skill `wechat-video-ppt-extract` is usable as the fixed post-video SOP for slide reconstruction. Treat the name as historical: in DataMax it should be generalized to uploaded/direct/public videos, not WeChat-specific automation. The reliable reusable portion is: frame capture or frame import -> contact sheet -> light PPT rectangle extraction -> conservative dedupe -> manual/model-assisted keep-list -> screenshot-based PPTX with manifest/source notes. The existing helper script does not directly ingest an `.mp4`; if DataMax already has a video file, first extract frames with FFmpeg/media worker into a `raw_frames`-style directory, then run the rectangle extraction/build steps and skip any recording step. The updated skill also defines an optional subtitle-overlay pass: OCR subtitle bands from original video frames, assign subtitles to pages using the pre-page rule, place text in a consistent box outside the PPT image area, and save transcript text plus JSON page mapping for later correction.

**Target flow:**

1. User uploads a video file, provides a direct video URL, or provides a public page where DataMax can safely resolve a video asset URL.
2. AssistantRun receives product/system briefing that explicitly lists `media.resolve_video_url`, `media.register_video_asset`, and `media.extract_ppt_transcript` as available DataMax-controlled actions.
3. The model decides whether to ask DataMax to parse the uploaded media directly or resolve/register a public video URL. It should not claim the video was accessed until DataMax returns a successful observation.
4. DataMax validates the action, registers the uploaded/resolved video as a user/session-scoped document or asset, and returns a concise progress step in the existing chat/right-shelf surfaces.
5. DataMax enqueues background media parsing. Foreground chat remains responsive.
...
2. Completed current slice: add DataMax action contracts: `media.resolve_video_url`, `media.register_video_asset`, and `media.extract_ppt_transcript`.
...
9. Completed current slice: generated video/PPT files in `video_extraction_summary.payload.generated_artifacts.files` can be discovered in the right-side generated project shelf and downloaded through a DataMax-validated route that only serves files inside the generated artifact workspace. This closes the first final-deliverable lifecycle gap for persisted discovery plus download and now marks complete generated packages with an immutable published-version manifest; durable history/object-store promotion remains separate.
...
- Resolved video is registered as a user/session-scoped DataMax document or asset and then parsed by background workers.
...
- If MiniMax/model/provider fails, DataMax reports the failure and preserves the user message; it does not fabricate the answer or pretend the video was accessed.
...
5. Allow interactive templates to emit only structured JSON patch or action-intent events back to DataMax.
...
9. Execute static-page planning handoff JSON patches only through DataMax static-page operations, not direct JSON/database mutation.
...
- Any user edit from an HTML artifact is converted into DataMax-validated JSON patch/action intent, never direct DOM/database mutation.
...
**Current slice:** Added the selected-source internal database status route and centralized observability lazy-load panel for database sources, plus the first usable MySQL incremental checkpoint loop and bounded paged row fetch. Operators can inspect config validity, default dataset binding, dataset Q&A readiness, per-table document/index/chunk readiness, recent sync workflow status, stage-level table sync counts, ingest document/row/chunk counts, skipped/failed row counters, capped row-failure samples, semantic-profile/mapping health findings, safe checkpoint summaries, redacted failure excerpts, persisted semantic profile summary counts, and a derived `sync_readiness` signal without exposing raw credentials, source cursors, or adding a separate always-on data observation page. Incremental sync can now reuse the latest successful checkpoint and filter by mapped updated-at, version, or numeric id high-water marks; row conversion failures are counted, sampled, persisted into sync-run counts, and shown in the centralized observability page without stopping the rest of the table fetch.

**Goal:** Add a safe MySQL database-source connector so an external database can be inspected, mapped, synchronized into an explicit DataMax dataset, parsed, indexed, and then used by existing dataset question-answering, report, template, and static-page workflows.

**Architecture:** Extend `external_source_connections`, `external-source-worker`, normal DataMax workflow state, and the main-system data-source page. Database sync must resolve a target dataset from the sync request or a configured default dataset, and ingestion must write database-derived documents under that dataset's normal visibility and indexing rules. Do not add database reads to chat endpoints. Do not expose database credentials to models or browsers.
...
5. Add MySQL source fetcher to `external-source-worker`, mapping configured rows into DataMax external document shape with `document_external_id`, `revision_external_id`, title, body, content type, and metadata; the workflow `dataset_id` remains the authoritative dataset membership.
...
9. Add the third-party database access contract in two phases: P0 managed/pre-provisioned source binding where DataMax operators configure credentials and third parties pass dataset scope; P1 self-service source registration with encrypted credential references, schema/profile/sync/status operations, and tenant-scoped responses.
...
- Keep database source data entering the model only after it has become DataMax documents/evidence through existing parse/index/retrieval visibility checks.
...
- DataMax can store and manage a MySQL source connection without storing raw credentials.
- Main system data-source page can test connection, inspect schema, preview allowed tables, configure mapping, bind/select a target DataMax dataset, start sync, and show status/drift.
- Operators can map one or more MySQL tables into DataMax external documents.
- Full sync and simple incremental sync create/update DataMax external documents and pass them through existing parse/index workflow.
...
- Third-party conversations use dataset scope or DataMax-resolved source-derived dataset scope for database-derived knowledge.
...
```text
Continue DataMax from docs/plans/2026-05-07-DataMax-master-development-plan.md.
...
Every model-facing turn must receive DataMax awareness as additive context: DataMax identity, product capabilities, visible permission-scoped datasets/tools, and unavailable evidence state. This must not restrict normal model ability; if DataMax has no visible evidence or permission for a topic, the answer should say it is `当前不可见/未供料` and may then continue with clearly labeled general knowledge.
Treat external/web search as a planned default read-only DataMax tool. Do not claim search results unless DataMax supplied audited search evidence with source and timestamp metadata.
