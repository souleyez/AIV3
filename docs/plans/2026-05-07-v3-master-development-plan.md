...
18. Partially completed current slice: render warning-specific video extraction completion actions and redacted completion-audit rows as readable labels inside the existing safe HTML artifact/right-shelf summary surface, including the contact-sheet/rectangle-extraction review action. Remaining UI artifact work must stay within the existing HTML artifact/right-shelf/main-chat surfaces and must not add another chat input, popup workbench, or global panel.
