...
- Video PPT/transcript extraction is partially present: V3 can parse uploaded/local audio-video evidence, preserve media timestamps, expose model-visible `resolve_video_url` / `extract_video_ppt_transcript` action contracts, recognize direct video URLs as remote media sources, auto-register a direct video URL into the selected visible dataset and enqueue ingest, let ReAct supply parsed transcript/scene/keyframe OCR evidence from selected uploaded video documents without host-composed answers, let `ingest-worker` download explicitly enabled direct remote media URLs into a guarded local cache before parsing, and now emits a safe read-only `video_extraction_summary` HTML artifact from parsed evidence into AssistantRun events/right-shelf discovery. Public page video resolution, full PPTX/Markdown extraction artifacts, and transcript/PPT artifact lifecycle are still pending. Login-gated video sites, QR login, cookies, and recording bypass flows are out of scope for the next implementation slice.
...
**Status:** In progress. This task turns the existing partial media foundation into a real assistant capability for direct video files and publicly resolvable video URLs/pages. Current baseline can parse uploaded/local media evidence, register direct video URLs, enqueue guarded remote-media ingest, and surface parsed video evidence as a safe read-only `video_extraction_summary` HTML artifact. It cannot yet reliably resolve a public page to a video asset or complete full PPTX/Markdown/source-text extraction as a durable product artifact.

**Capability boundary:**

- Already exists: generic audio/video upload ingestion, direct video URL registration, guarded remote-media ingest, media metadata/transcript/scene/keyframe evidence surfaces, media detail API, timestamp-aware retrieval/static-page supply, model-visible ReAct action contracts for URL resolution and video PPT/transcript extraction, and safe HTML artifact contracts/templates for displaying extraction summaries.
- Missing: a public-page video resolver, full PPT/transcript extraction workflow, generated PPTX/Markdown/source-text artifacts, and artifact lifecycle for final deliverables.
