...
- Missing: durable published-version history beyond the generated artifact package, stronger visual rectangle detection/crop replacement beyond the current conservative contrast/edge heuristics, richer native PPTX rendering, and deployment/smoke enablement for the new assistant-run worker service.
...
8. `node --test apps/web/app/lib/html-artifact-manifest.test.mjs`
9. `pnpm --filter @ai-data-platform-v3/web build`
10. Resolver smoke only after safe stubs pass: direct MP4 fixture, simple public HTML `<video>` fixture, unsupported/login-required fixture, and blocked/private URL fixture.
