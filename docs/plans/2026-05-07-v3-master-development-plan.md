...
- Missing: durable published-version lifecycle for final deliverables, rectangle extraction/dedupe promotion, richer PPTX/Markdown rendering, model-authored completion turn wiring after background extraction, and richer failure handling for unsupported source/unavailable video/resolver failures.
...
17. Partially completed current slice: surface raw frame extraction `failed` / `skipped` states, generated-artifact writer failures, parse-partial evidence, provider failures, and low-confidence transcript/OCR evidence as structured deliverable warnings with reason/count/missing-evidence/provider details, and map those plus core review warnings into completion follow-up next actions. Remaining failure handling: unsupported source, unavailable video, resolver blocked, and no slide rectangle found.
