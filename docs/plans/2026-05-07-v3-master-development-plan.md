...
17. Partially completed current slice: surface raw frame extraction `failed` / `skipped` states and generated-artifact writer failures as structured deliverable warnings with reason text, and map those warnings into completion follow-up next actions. Remaining failure handling: unsupported source, unavailable video, resolver blocked, parse partial, provider failure, no slide rectangle found, subtitle OCR low confidence, and low-confidence extraction.
18. Add UI artifact updates only inside the existing HTML artifact/right-shelf/main-chat surfaces. Do not add another chat input, popup workbench, or global panel.
