...
15. Partially completed current slice: generated `source_text.md` and `ppt_outline.md` now include transcript/scene/OCR evidence refs, confidence/source labels, and redacted provider capability status; richer transcript segmentation, keyframe clustering, OCR slide text promotion, PPTX rendering, and source-text artifact expansion remain pending.
