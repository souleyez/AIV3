...
- Codex plan-only action suggestions and the real static-page preview queue now consume static-page `bindingQuality` summaries: if chart/module data still needs attention, the executor or backend gate suggests module repair or retrieval instead of submitting an effect-image preview; confirmed data quality still allows the normal `效果图——生成页面` path. The web request layer preserves structured gate details so module-level repair hints are not lost before reaching the UI.
