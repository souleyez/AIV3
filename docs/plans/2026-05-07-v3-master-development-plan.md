...
18. Partially completed current slice: render warning-specific video extraction completion actions as readable labels inside the existing safe HTML artifact/right-shelf summary surface, including the contact-sheet/rectangle-extraction review action. Remaining UI artifact work must stay within the existing HTML artifact/right-shelf/main-chat surfaces and must not add another chat input, popup workbench, or global panel.
19. Partially completed current slice: video extraction workflow-completed events now include a redacted `completion_audit` with output/deliverable states, artifact kinds, warning codes, and provider-failure counts, without copying cookies, private URLs, raw tokens, provider keys, or local source paths.
