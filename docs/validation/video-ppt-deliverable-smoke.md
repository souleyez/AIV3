...
## 2026-06-08 Second Public Sample Current-Head Revalidation

Task source: M6BQ validation-only follow-up. Existing `Layered Nix Stores` public-course deliverables under `target/` were available from prior extraction runs. This slice revalidates that second strong public sample against the current HEAD without downloading video again, uploading anything, running live endpoints, extracting frames, or regenerating PPT artifacts.

Scope:

- validation-only second public-course deliverables review;
- no code change required for this slice;
- no live main-site upload;
- no third-party live event;
- no bearer/context use;
...
```text
node tools/validate-video-deliverables.mjs <layered-public-generated-artifacts> --json
npm run smoke:video-ppt-quality-matrix -- --public-course-deliverables <layered-public-generated-artifacts> --pretty --output-dir <target-redacted>
node -e "<redacted layered public quality matrix readback>"
rg -n "<local-path-url-token-patterns>" <target-redacted>
git diff --check
git ls-files target | wc -l
```
...
- validator output redacted the artifacts directory and file paths;
- checked file kinds included PPTX, final manifest, published manifest, version history, extraction manifest, slide rectangles manifest, selected slides manifest, slide notes, `video_slides.md`, and slide quality report;
- selected slide count was `3`;
- requested selected count was `4`;
- PPTX slide count was `3`;
- Markdown slide count was `3`;
- slide rectangle count was `3`;
- quality slide count was `3`;
- subtitle page map remained absent, which is valid for this no-subtitle/no-transcript public sample contract;
- public-course quality matrix returned `status=partial_public_course_deliverables_reviewed`;
- `case_count=3`, `deliverable_count=1`, `needs_manual_review_count=1`, `not_deliverable_count=0`, and `pending_count=1`;
- public course case category was `public_course_video`;
...
- public deliverable state was `final_pptx_ready`;
- public validator status was ok;
- public quality score was `68`;
- public risk flags were `missing_transcript_alignment`, `selected_slide_duplicates_removed`, and `manual_review_required`;
- public quality summary recorded `full_frame_fallback_count=0`, `detector_crop_count=3`, `subtitle_missing_count=3`, `ocr_missing_count=3`, `sharpness_high_count=0`, `readability_high_count=0`, and `single_slide_output=false`;
- public evaluation verdict was `needs_manual_review`;
- public failure class was `selection_quality`;
- customer-authorized case stayed pending;
...
- this does not replace main-site upload live smoke, third-party live smoke, video号 handoff live pass, authorized capture live sample, 8-server deployment validation, or customer-authorized quality matrix;
- this confirms the second strong public sample remains a real public-course `needs_manual_review` case, not a clean customer deliverable.
