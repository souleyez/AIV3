...
- P1-8 只证明检索级 ranking 修复；P1-11 本地 fixture 已证明 AssistantRun 能把预期证据供给到模型侧；P1-12A 本地 fixture 已证明客户答案 evaluator 能拦截模板副作用、内部字段泄露和普通问答误路由，但还不等于真实 provider/live customer-answer 质量闭环。
...
- [x] 补 NewBai customer-answer 离线 smoke，覆盖最终答案 pattern、证据使用、内部字段泄露、false report/page claim、已有模板复用、普通问答误路由、数据库+文档混合边界。
- [x] 补 NewBai customer-answer live-capture harness，默认 self-test/preflight 不发网络请求，live 模式要求 `--run-live --ack-controlled-live` 和 bearer/dataset scope。
- [ ] 用受控 provider/live result JSONL 跑 NewBai customer-answer evaluator，记录真实最终答案质量 receipt。
...
P1-11: provider-input deterministic smoke 和一次性 Postgres fixture AssistantRun DB route smoke 已补
P1-12A: NewBai customer-answer 离线 evaluator 已补
P1-12B: live-capture harness 已补；下一步用受控 provider/live result JSONL 补真实最终答案质量 receipt
P1-12C: 真实答案 receipt 通过后，再评审已有模板绑定/页面复用守卫是否需要 Rust 侧硬门禁
P1-13: 通过后再申请 8 服务器 `RETRIEVAL_SEARCH_BACKEND=postgres_lexical` 小流量灰度
```
