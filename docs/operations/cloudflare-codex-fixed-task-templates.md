...
- Live enqueue is explicitly enabled with `ASSISTANT_RUN_ANSWER_QUALITY_AUTOFIX_ENABLED=true`.
...
For `answer_quality_autofix`, also set `ASSISTANT_RUN_ANSWER_QUALITY_AUTOFIX_ENABLED=false` to keep passive collection on while preventing live Codex Host task creation.
