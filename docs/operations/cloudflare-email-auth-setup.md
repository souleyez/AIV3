...
This project uses Cloudflare Email Service as the server-side email delivery channel for verification codes and bounded operational exception notices. DataMax still owns users, sessions, OTP challenges, ownership checks, audit records, and operational routing policy.
...
- DataMax already generates and stores the OTP challenge.
...
6. Configure DataMax server environment variables.

## DataMax Environment Variables
...
If the required variables are absent, DataMax falls back to `LoggingEmailSender`. This keeps local development and tests usable without Cloudflare credentials.

## Security Rules

- DataMax must never email the raw local key.
- DataMax must never store the raw local key.
...
- Frontend talks only to DataMax `/v1/auth/*`; it never calls Cloudflare directly.

## Failure Behavior

- Cloudflare send failure returns `验证码发送失败，请稍后再试`.
- DataMax creates the OTP challenge only after email sending succeeds.
