...
async fn main() -> anyhow::Result<()> {
    observability::install("report_read_published_cli")?;

    let program = std::env::args()
        .next()
        .unwrap_or_else(|| "report-read-published-cli".to_string());
    ...
    let response = platform_api::load_published_report_by_plan(storage, tenant.id, plan_id)
        .await
