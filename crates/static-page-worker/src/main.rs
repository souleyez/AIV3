...
use chrono::Utc;
use domain_model::{
    StaticPageDraftStatus, StaticPageImageJob, StaticPageImageJobStatus, WorkflowKind,
};
...
const DEFAULT_TASK_KEY: &str = "generate_static_page_image";
...
async fn main() -> Result<()> {
    ...
    let task_key = std::env::var("STATIC_PAGE_IMAGE_TASK_KEY")
        .unwrap_or_else(|_| DEFAULT_TASK_KEY.to_string());
    ...
    let mut task_waker = event_bus
        .subscribe_queue_or_disabled(
            &wake_subject,
            Some(&format!("static_page_worker.{queue}.{task_key}")),
        )
        .await;
