...
- `npm run smoke:video-ppt-quality-matrix`: deterministic P2-2E quality-matrix self-test. It records the required synthetic PPT playback, public course video, and customer-authorized video review categories, marks only the synthetic fixture as locally deliverable, and keeps public/customer samples pending until approved inputs exist. Default use is `-- --self-test`; it does not call DataMax, download video, upload files, record browser sessions, or deploy services.
