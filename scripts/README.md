...
- `npm run smoke:video-ppt-quality-matrix`: deterministic P2-2E quality-matrix self-test and local deliverables review. Use `-- --self-test` to check the fixed synthetic/public/customer matrix shape, or `-- --synthetic-deliverables target/<video-extraction>/generated_artifacts` to validate a local generated-artifacts package through `validate-video-deliverables` and classify only the synthetic case from that evidence. Public-course and customer-authorized cases stay pending until approved inputs exist. It does not call DataMax, download video, upload files, record browser sessions, or deploy services.
