...
- `npm run smoke:external-video-ppt`: third-party video/PPT extraction smoke. It registers a public or loopback video fixture through `/v1/external/channels/{connection_id}/documents/parse`, sends a scoped `/events` message that explicitly asks to extract PPT/slides/courseware already shown in the video, polls the assistant-run reply, and validates the third-party video extraction card plus optional PPTX/Markdown/manifest downloads. Use `-- --self-test` for the deterministic offline reply-surface contract; live mode requires an approved inbound bearer.
