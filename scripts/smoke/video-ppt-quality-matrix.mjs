...
function buildSelfTestReport() {
  const cases = buildSelfTestCases().map((testCase) => {
...
function summarizeCases(cases) {
  ...
  for (const testCase of cases) {
    ...
    if (!testCase.expectation_matched) {
...
function validateSelfTestReport(report) {
  ...
  if (!report.all_required_categories_present) {
    throw new Error('self-test report must cover all required P2-2E categories');
  }
  ...
  if (
    ...
    throw new Error('self-test report safety gates are invalid');
  }
}
...
function writeReport(outputDir, report, pretty) {
  ...
  const reportPath = path.join(outputDir, `${makeRunId()}-self-test.json`);
...
function main() {
  ...
  if (!args.selfTest) {
    throw new Error('--self-test is required; live P2-2E matrix runs need explicit approved samples');
  }
  const report = buildSelfTestReport();
  const reportPath = writeReport(args.outputDir, report, args.pretty);
  console.log(
    `OK video PPT quality matrix self-test: cases=${report.summary.case_count} deliverable=${report.summary.deliverable_count} pending=${report.summary.pending_count} report=${reportPath}`,
  );
}
