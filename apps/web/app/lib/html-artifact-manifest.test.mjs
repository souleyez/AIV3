...
test('renders third-party handoff document template', () => {
  const handoff = renderHtmlArtifactDocument(baseManifest({
    id: 'third-party-handoff-doc',
    sourceType: 'external_integration',
    templateId: 'third_party_handoff_document',
    title: '纯第三方模式对接文档',
    payload: {
      handoff: {
        status: 'review_ready',
        ...
        sourceDocument: 'pure-third-party-integration-guide.zh-CN.md',
        reviewDocument: 'pure-third-party-integration-guide.zh-CN.html',
      },
      summary: '第三方自建页面、文档库、用户权限、产物与业务接口，V3 负责权限供料、模型回复、动作确认和审计。',
      integrationModes: [{
        title: '纯第三方模式',
        detail: '第三方托管聊天页面、文档库、用户中心和业务系统。',
        status: 'ready',
      }, {
        title: '混合模式',
        detail: '聊天面在飞书或企微，知识和动作来自第三方服务器。',
        status: 'planned',
      }],
      endpoints: [{
        method: 'POST',
        path: '/v1/external/channels/{connection_id}/events',
        description: '接收第三方聊天页面标准消息事件。',
        auth: 'channel signing',
      }, {
        method: 'POST',
        path: '/v1/external/channels/{connection_id}/actions/{action_id}/result',
        description: '接收异步动作结果回调。',
        auth: 'result callback signing',
      }],
      deliveryArtifacts: [{
        title: 'HTML 阅读版',
        path: 'docs/pure-third-party-integration-guide.zh-CN.html',
        description: '用于业务和技术评审。',
      }],
      validationCommands: [{
        title: '一键交接校验',
      ...
      reviewChecklist: [{
        title: '权限样例',
        detail: '确认用户、部门、组和 ACL 样例覆盖可见/不可见场景。',
        owner: 'third_party',
      }],
      safetyRules: [{
        title: '不搬迁整库',
  ...
  assert.match(handoff.html, /第三方交接文档/);
  assert.match(handoff.html, /外部集成/);
  assert.match(handoff.html, /v3\.elepcloud\.com/);
  assert.match(handoff.html, /POST \/v1\/external\/channels\/\{connection_id\}\/events/);
  assert.match(handoff.html, /动作结果回调/);
  assert.match(handoff.html, /HTML 阅读版/);
  assert.match(handoff.html, /npm run validate:all/);
  assert.match(handoff.html, /不搬迁整库/);
});
...
test('renders video extraction summary template', () => {
  ...
  assert.match(video.html, /复核字幕对页/);
