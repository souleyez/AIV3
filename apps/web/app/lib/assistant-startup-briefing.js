...
export function buildAssistantStartupBriefing({
  ...
  return {
    ...
    productCapabilities: {
      staticPage: '可以在主对话区发起静态页规划、确认可视化文案、可视化排队、最终静态页渲染，并导出 index.html 与包含 manifest/data/modules/render-spec/runtime/README 的 ZIP 交付包。高级静态页默认按 Image2 视觉稿 -> 真实数据 HTML -> 快照/单位口径校验 -> DataMax 产物发布执行；快照表当前状态只取最新快照，趋势图才跨日期展开。',
...
function formatStaticPageWorkspaceForModel(workspace) {
  ...
  if (workspace.activeDraftId) {
    parts.push(
      `当前静态页：${workspace.activeDraftObjective || workspace.activeDraftId}，状态 ${workspace.activeDraftStatus || 'unknown'}，风格 ${workspace.activeStyleDirection || '未定'}，模块 ${workspace.activeModuleCount || 0} 个，ECharts ${workspace.activeEchartsModuleCount || 0} 个，可视化 ${workspace.previewStatus || 'none'}，最终渲染 ${workspace.finalRenderStatus || 'none'}。`,
    );
    if (workspace.canEditModules) {
      parts.push('用户要求调整静态页时，应优先围绕当前打开草稿做模块级标题、内容、数据、图表、布局或风格变更。');
    }
    if (workspace.previewStale) {
      parts.push('当前规划已变更，旧可视化和最终页不能继续复用，应先重新生成可视化。');
    }
...
function formatStaticPageDataQualityForModel(quality) {
  ...
  if (quality.status === 'attention_required') {
    ...
    return [
      ...
      '在提交可视化或最终渲染前，应优先让 DataMax 检索/细读/修复模块数据；没有 DataMax 供料时必须说明当前不可见/未供料，不能编造图表数据。',
    ].filter(Boolean).join('。');
  }
  if (quality.status === 'unknown') {
    return `静态页数据质量：当前有 ${quality.moduleCount || 0} 个模块，但没有模块绑定质量快照；提交可视化前应先刷新 dataSnapshot 或补齐模块数据。`;
  }
  if (quality.status === 'ready') {
    return `静态页数据质量：当前 ${quality.readyModuleCount || 0} 个模块绑定未发现阻塞，可继续可视化或最终渲染；仍需按 DataMax 已供料证据回答。`;
  }
  return '';
}
