...
export function promptMayUseCustomerCodex(prompt) {
  ...
  const hasV3ProductSignal = [
    ...
    || ['v3', 'api', 'auth', 'deploy', 'login'].some((signal) => lower.includes(signal));
  const hasProductChangeSignal = [
    '修改',
    '改造',
    '调整',
    '删除',
    ...
    || ['modify', 'change', 'patch', 'deploy', 'restart'].some((signal) => lower.includes(signal));
  if (hasV3ProductSignal && hasProductChangeSignal) {
    return true;
  }
  return [
    'Codex',
    ...
    || ['codex', 'dashboard', 'visualization', 'analytics', 'report', 'artifact', 'webpage', 'script', 'executor'].some((signal) =>
