...
| 视频 PPT 抽取 | 先通过文档解析接口上传/登记视频文件，或在普通消息中提供可匿名下载的直接视频 URL；再用普通消息提出“提取视频里的 PPT/幻灯片/课件”。微信视频号或登录态来源应提示上传文件、提供直链或申请 operator 授权录屏处理 | `reply.card.type`、`reply.card.status`、`reply.card.download_exports[]`、`reply.artifact_links[]` | `video_extraction_queued`、`processing`、`video_extraction_summary`、`failed`、`login_gated_video_source_not_supported` |
