...
test('buildReadinessReport fails closed when final evidence markdown receipt is stale', () => {
  const report = buildReadinessReport({
    requests: [
      {
        bearer_valid: true,
    ...
    handoffEvidenceValidation: {
      ...
      evidence_markdown_receipt: {
        ...
        errors: [
          {
            code: 'evidence_markdown_receipt_sha256_mismatch',
            message: 'evidence markdown receipt SHA256 does not match validation output',
            path: 'target/external-third-party-handoff/package.evidence-manifest.md',
          },
        ],
      },
      errors: [],
